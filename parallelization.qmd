...
> Quantity has a quality all its own. - Apocryphal; popularly attributed to Joseph Stalin
...
#| output: false
using CairoMakie
# Reset Makie theme to clear deprecated `resolution` keyword
# injected by Quarto's IJulia setup.jl (see quarto-dev/quarto-cli#7817)
set_theme!()
```
...
Parallel processing comes in different flavors and is related to the details of hardware as discussed in @sec-hardware. We will necessarily extend the discussion of hardware here, as parallelization is (mostly) inextricably tied to hardware details. We revisit this in @sec-parallel-programming-models when we connect the techniques back to programming abstractions and real models.
...
**Vectorization**`\index{vectorization}`{=latex} in the context of parallel processing refers to special circuits within the CPU wherein the CPU will load multiple data units (e.g. 4 or 8 floating point numbers) in a contiguous block and perform the same instruction on them at the same time. This is also known as **SIMD, or Single-Instruction Multiple Data**`\index{SIMD}`{=latex}. Think of it like a `for` loop where each iteration does four or eight operations at a time instead of one.

The requirements for SIMD-able code are that:

-   The intended section for SIMD is inside the inner-most loop.
-   The loop body is type-stable and free of complex control flow: cross-iteration dependencies, early exits, and functions that cannot be inlined all prevent vectorization. (The compiler can often convert a *simple* branch, like the `if` in the example below, into a branchless "masked" operation.)
-   Reordering the iterations must not change the result — or the programmer must explicitly permit reordering, as discussed below.
...
function sum_positive_simd(arr)
    s = zero(eltype(arr))
...
let
    ...
    @btime sum_positive($x)
    @btime sum_positive_simd($x)
end
```

In testing the above code, the `sum_positive_simd` version should be several times faster than `sum_positive`. Perhaps surprisingly, the branch is not the obstacle here: the compiler converts the simple `if x > 0` into a branchless masked operation in both versions. What actually prevents `sum_positive` from vectorizing is the summation itself. Floating point addition is not associative — accumulating the elements four or eight at a time produces a (very slightly) different result than adding them one-by-one in order, and the compiler will not silently change the answer your code would produce. The `@simd` macro is our explicit permission to reorder the accumulation, which unlocks vectorization (enter `?@simd` in the REPL for details, and see @sec-optimizing-SIMD for more).

::: callout-note
Note that the accumulator is initialized with `zero(eltype(arr))` rather than the integer literal `0`. With `s = 0`, the accumulator would start as an `Int` and only be promoted to `Float64` once the first positive element is added, making the loop body type-unstable (see @sec-performance-single) — which by itself is enough to defeat vectorization and slow the loop several-fold, independent of any branching.
:::

Other types of parallelism that we will discuss in this chapter have some risk of errors or data corruption if not used correctly. *Automatic* vectorization isn't prone to issues like this, because the compiler only vectorizes code when it can prove the transformation does not change the result. Annotations shift that responsibility to the programmer, however. `@simd` is a promise to the compiler that iterations may be safely reordered — including reordering floating point reductions, which can change results in the last few digits — and applying it to a loop with cross-iteration dependencies can produce silently incorrect results. Third-party macros like `@turbo` (from LoopVectorization.jl) are more aggressive still and carry the same caveat (e.g., `x[i] = x[i-1]` inside the loop).

### Hardware

Vectorization is hardware dependent. If the CPU does not support vectorization you will not see speedups from it. Many consumer and professional chips have AVX2`\index{AVX2}`{=latex} (Advanced Vector Extensions 2), which uses 256-bit registers allowing four simultaneous 64-bit floating-point operations. AVX-512`\index{AVX-512}`{=latex}, with 512-bit registers (eight simultaneous 64-bit operations), is common on Intel server chips and recent AMD processors. However, AVX-512 can cause thermal throttling on some chips—wider SIMD uses more power and generates more heat—so real-world speedups depend on sustained workloads and cooling.

On ARM processors (AArch64), the equivalent is NEON`\index{NEON}`{=latex} (128-bit, two 64-bit operations) and the newer SVE/SVE2`\index{SVE (Scalable Vector Extension)}`{=latex} (Scalable Vector Extension) which supports variable-width vectors up to 2048 bits. Apple's M-series chips use NEON; ARM server chips like AWS Graviton3 support SVE.
...
To understand multi-threading examples, we first need to discuss **Tasks**`\index{tasks}`{=latex}, which are chunks of computation that get performed together, but after which the computer is free to switch to a new task. Technically, there are some instructions within a task that will let the computer pause and come back to that task later (such as `sleep`).

Tasks do not, by themselves, allow for multiple computations to be performed in parallel. For example, one task might be loading a data file from persistent storage into RAM. After that task is complete, the computer continues on with another task in the queue (rendering a web page, playing a song, etc.). In this way even a single processor computer could be "doing multiple things at once" (or "multi-tasking") even though nothing is running in parallel. The scheduling of the tasks is handled automatically by the language runtime or the operating system.
...
let
    ...
    @sync begin # <5>
        for i in 1:5
            @async trade_producer(channel, i) # <6>
        end
...
When a program starts on your computer, the operating system creates a **process**`\index{process}`{=latex}. It allocates bookkeeping structures (to track code, stack frames, and heap allocations) and reserves a block of memory in RAM for that process. Different processes do not have access to each other's allocated memory.
...
- **Operating-system threads** are managed by the kernel, which schedules them onto the available hardware threads. They are heavier to create and destroy than Julia tasks.
...
The first issue is known as a **race condition**`\index{race condition}`{=latex}, which occurs when a block of memory has been read from or written to in an unintended order. For example, if we have two threads which are accumulating a sub-total, each process may read the running sub-total before the other thread has finished its update.
...
3.  One thread writes its result back to `subtotal` first.
...
function sum_chunker(a)
    ...
    chunk_sums = fetch.(tasks) # <3>

    return sum_single(chunk_sums) # <4>
...
1.  Split `1:a` into roughly equal ranges across available threads.
2.  Launch a task for each chunk to compute a single-threaded sum.
3.  Fetch each task's result once it finishes.
4.  Aggregate the partial sums on a single thread; this is cheap because there are only as many partial sums as threads.

##### Using Locks

**Locks**`\index{locks}`{=latex} prevent memory from being accessed from more than one thread at a time.
...
**Atomics**`\index{atomics}`{=latex} are certain primitive values with a reduced set of operations for which Julia and the compiler can automatically create thread-safe code. This is often significantly faster than the context-switching overhead needed with locking and unlocking memory for threaded tasks. Compared with locks, atomics are simpler to implement and easier to reason about. The downside is that atomics are limited to the available primitive atomic types and methods.
...
|--------|-------------------|-------------|
| NVIDIA | GeForce RTX (consumer), Quadro/RTX A-series (professional), A100/H100/H200 (data center) | CUDA`\index{CUDA}`{=latex} |
| AMD | Radeon (consumer), Radeon Pro (professional), Instinct MI series (data center) | ROCm`\index{ROCm}`{=latex} (HIP) |
| Intel | Arc (consumer), Data Center GPU Max / Ponte Vecchio (data center) | oneAPI (SYCL) |
| Apple | M-series integrated GPU (M1, M2, M3, M4) | Metal`\index{Metal}`{=latex} |
| Google | TPU v4, v5 (cloud only) | XLA (via JAX or TensorFlow) |

: GPU and TPU vendors with representative hardware and programming interfaces. NVIDIA dominates the data center market; Apple Metal is only available on macOS/iOS.
...
There are essentially two types of GPU programming we will discuss here:

1.  **Array-based programming**`\index{array-based programming}`{=latex}, where arrays are stored and operated on directly on the GPU memory. This approach abstracts away the low-level details, allowing you to work with familiar array operations that are automatically executed on the GPU.
2.  **Custom kernels**`\index{kernels (GPU)}`{=latex}, which are specialized functions that define exactly how each GPU thread should process data in parallel. A kernel explicitly specifies the computation that each GPU thread will perform on its portion of the data.
...
A third approach would be to implement GPU code in a low-level vendor toolkit (such as C++ and associated CUDA libraries), but this approach will not be illustrated here.
...
@btime Metal.@sync calculate_present_values!($pvs_GPU, $cashflows_GPU, $discount_matrices_GPU) # <6>
```
...
3.  Metal only supports 32-bit floating point (`Float32`) in GPU shaders. NVIDIA data center GPUs (A100, H100) have full-speed FP64; consumer GPUs (RTX series) have heavily throttled FP64 (often 1/64th the FP32 rate). For financial calculations requiring double precision, this is an important hardware consideration.
4.  Using 100 thousand scenarios for this example.
5.  `MtlArray(array)` will copy the array values to the GPU.
6.  Note that the data still lives on the GPU and is of the `MtlMatrix` type (an alias for a 2-D `MtlArray`). Also note the `Metal.@sync`: GPU operations are launched asynchronously, so we must wait for the GPU to actually finish its work — without it, the benchmark would mostly measure the cost of *queueing* the work rather than performing it.
...
function calculate_present_values_kernel!(present_values, cashflows, discount_matrices)
    ...
    if idx <= size(discount_matrices, 2) # <2>
        pv = 0.0f0 # <3>
        for t in 1:size(cashflows, 1)
            pv += cashflows[t] * discount_matrices[t, idx] # <4>
        end

        present_values[idx] = pv # <5>
    end
    return nothing # <6>
end
```
...
2.  The launch configuration below rounds the thread count up to a whole number of threadgroups, so a few trailing threads will receive an `idx` beyond the number of scenarios; the bounds check keeps them from reading or writing out of bounds.
3.  Recall that we are working with `Float32` on the GPU here, so the zero value is set via the `f0` notation indicating a 32-bit floating point number.
4.  The loop is across timesteps within each thread, while the thread index is tracked with `idx`.
5.  The result is written to the pre-allocated array of present values, and we avoid race conditions because the different threads are working on different scenarios.
6.  We don't explicitly have to `return nothing` here, but it makes it extra clear that the intention of the function is to mutate the `present_values` array given to it. This mutation intention is also signaled by the `!` convention in the function name.

The kernel above was fairly similar to how we might write code for CPU-threaded approaches, but we now need to specify the technicals of launching this on the GPU. The `threads` argument defines how many threads run within each *threadgroup*; threads in the same group execute together, can share fast threadgroup memory, and can synchronize with one another, and the maximum group size depends on the hardware used. The `groups` argument defines how many such groups are launched — together, `threads × groups` determines the total number of GPU threads. The push-pull here is that threads that can share data avoid needing to create duplicate copies of that data in memory. However, if there is variability in how long each calculation will take, then the waiting time for synchronizing results may slow the overall computation down.

Through some experimentation in advance, we find that a relatively large group size of \~512 works well for this kernel.

We bring this all together through the use of the kernel macro `@metal`:

```{julia}
threads = 512
groups = cld(num_scenarios, threads)

@btime Metal.@sync @metal threads = $threads groups = $groups calculate_present_values_kernel!(
    $pvs_GPU,
    $cashflows_GPU,
...
-   As in the Metal version, the kernel needs a bounds check (`if idx <= ...`) because `threads × blocks` will typically overshoot the data size. For example, `cld(100_000, 1024)` gives 98 blocks, launching 100,352 threads for only 100,000 scenarios.
...
Multiple device, or **multi-device**`\index{multi-device computing}`{=latex}, computing refers to using separate groups of memory/processor combinations to accomplish tasks in parallel. This can be as simple as multiple distinct cores within a single desktop computer, or many separate computers networked across the internet, or many processors within a high performance cluster or a computing-as-a-service provider like Amazon Web Services or JuliaHub.
...
-   `myid()` is the identification number of the given process that's been spun up.
-   We use a `RemoteChannel` instead of a `Channel` to facilitate communication across processors.
-   Instead of `@async`, we use `@spawnat n` to create a task for process number `n` (or `:any` will automatically assign a process).
...
Map-Reduce`\index{map-reduce}`{=latex} (@sec-map-reduce) operations are inherently parallelizable and various libraries provide parallelized versions of the base `mapreduce`. This is the workhorse function of many 'big data' workloads, and many statistical operations are versions of `mapreduce`.

#### Multi-Threading

##### OhMyThreads

[OhMyThreads.jl](https://github.com/JuliaFolds2/OhMyThreads.jl) provides the threaded versions of essential functions such as `tmap`, `tmapreduce`, `tcollect`, and `tforeach` (see @tbl-functional-methods). In most cases, the chunking and data sharing is handled automatically for you.
...
The difference between the two approaches is how the work is assigned. `pmap` dynamically sends each element of `collection` to the next available worker as it finishes its prior work, which balances load well when each call of `f` is expensive but incurs communication overhead on every element. `@distributed` statically partitions the iteration range up front into one contiguous block per worker, which has minimal per-iteration overhead and suits cheap loop bodies. (Note that any outside variables referenced inside the `@distributed` loop body — including a collection that is indexed into — are serialized to every worker.)
...
# fast and the per-element communication overhead dominates
...
Using multi-processing`\index{SharedArrays}`{=latex}:
...
- @rackauckas2020parallelism provides an excellent overview of parallelism types in the context of scientific computing
- @juliadocs2024parallel is the official Julia documentation on parallel computing constructs
- @enccs2023juliahpc offers hands-on tutorials for high-performance computing with Julia
