...
::: callout-note
A second effect compounds the branch in `prevent_simd`: the accumulator is initialized to the integer `0`, so whether it has been promoted to a `Float64` yet depends on the branch history, and the loop body is not type-stable (see @sec-performance-single). The branchless `allow_simd` settles into a single concrete type after the first iteration and vectorizes. Modern compilers can sometimes rewrite a *simple* branch into branchless code on their own, but it takes little — a type that changes across iterations as here, an early exit, or a more complex loop body — to defeat both that rewrite and vectorization.
:::

::: callout-tip
Note that Julia's compiler is able to identify vectorizable code in many cases, though some cases may benefit from a more manual hint to the compiler through macro annotations (enter `?@simd` in the REPL for details). See @sec-optimizing-SIMD for more.
:::
