...
Implied rates are a form of model inversion`\index{model inversion}`{=latex}: given an observed price, what rate would produce that price? If your pricing function and your implied-rate function don't round-trip consistently, something is wrong.
...
**Mediators**`\index{mediators}`{=latex} sit on the causal pathway. A capital rule affects lending supply, which affects loan growth. If you control for lending supply, you block part of the effect you're trying to measure.

**Colliders**`\index{colliders}`{=latex} are caused by two other variables. Regulation intensity and market stress both affect media coverage. If you control for media coverage, you *create* a spurious correlation between regulation and stress.
