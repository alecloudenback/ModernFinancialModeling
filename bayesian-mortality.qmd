...
\begin{aligned}
q &\sim \text{Beta}(1,1) \\
\text{deaths} &\sim \text{Binomial}(n, q)
\end{aligned}
$$
...
The **prior**`\index{prior (Bayesian)}`{=latex} distributions for `a`, `b`, and `k` are chosen to constrain the hazard (mortality) rate to be between zero and one (`c` is fixed at zero in this model; it becomes a sampled parameter in the final model below).
...
```{julia}
predicted_deaths = vec(mean(Array(preds); dims=(1, 3)))
