...
An example of using a Bayesian\index{Bayesian inference}`{=latex}` MCMC\index{Markov chain Monte Carlo (MCMC)}`{=latex}` approach with Turing.jl\index{Turing.jl}`{=latex}` to fitting a mortality curve to sample data, with multi-level models and censored data.
...
Estimate $q$, the average mortality rate, not accounting for any variation within the population/sample. Our model is defined as:
...
This model extends the prior to create a multi-level model\index{multi-level model}`{=latex}`. Each risk class (`risk_level`) gets its own $a$ parameter in the `MakehamBeard` model. The prior for $a_i$ is determined by the hyper-parameter $\bar{a}$.
