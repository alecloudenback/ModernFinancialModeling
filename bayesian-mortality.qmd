...
An example of using a Bayesian\index{Bayesian inference}`{=latex}` MCMC\index{Markov chain Monte Carlo (MCMC)}`{=latex}` approach to fitting a mortality curve to sample data, with multi-level models and censored data.
...
using Turing\index{Turing.jl}`{=latex}`
