...
The reliability of modeled outcomes is fundamentally dependent on the quality of both the underlying scenarios and the model framework itself. For example, say that your model of interest rates produces a range of short rates that is very narrow (relative to historical values or implied volatiles). If you then run risk metrics using scnearios with unrealistically narrow short rates, you may find false comfort in the small world (@sec-small-large-world) of your model.
