...
The quality of modeled outcomes is closely tied to the scenarios and the model itself. For instance, there have been instances where economic scenario generators, such as the American Academy of Actuaries' Economic Scenario Generator used for US regulated insurance liabilities, produced interest rate paths that were notably higher than those implied by current market conditions. In such cases, if labeling the 99th-percentile scenario as "extreme" needs care would need to be taken to appropriately describe what exactly is meant by that label. Additionally, it's important for modelers to be aware of any potential limitations or areas for improvement in the model's behavior when interpreting results.
...
In this approach, you first generate many sets of random inputs (scenarios) and then run the same model repeatedly—one run per scenario[^stochastic-2]:

[^stochastic-2]: An extended example of this approach is discussed in @sec-scenario-generation.
...
Here, the model itself “rolls the dice” at each step. Instead of pre-generating input scenarios, the randomness is embedded in the logic that evolves the state over time[^stochastic-3]:
