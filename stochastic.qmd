...
Despite the opening quote's humorous skepticism, **Monte Carlo methods**`\index{Monte Carlo methods}`{=latex} have become indispensable in modern financial modeling. Named after the famous casino, these techniques use repeated random sampling to obtain numerical results when analytical solutions are impractical or impossible.
...
A stochastic model's **state space**`\index{state space}`{=latex} represents all possible states the system can occupy. This might be:
...
- Aleatory uncertainty`\index{aleatory uncertainty}`{=latex}: The inherent randomness in the system (e.g., market volatility)
- Epistemic uncertainty`\index{epistemic uncertainty}`{=latex}: Uncertainty due to limited knowledge about model parameters
...
A stochastic process is a collection of random variables indexed by time (or some other variable) that evolve in a probabilistic manner. Examples include Brownian motion, Markov chains and Poisson processes`\index{Poisson process}`{=latex}. Stochastic processes model how random variables change over time, which is essential for understanding dynamic systems influenced by randomness.
...
$$
where $s$ is the sample standard deviation and $N$ is the scenario count. Doubling accuracy therefore requires roughly four times as many runs. Plotting running estimates (mean, VaR, CTE) versus scenario count is an easy convergence diagnostic; once the curve flattens relative to your materiality threshold, stop the Monte Carlo.
...
Risk measures`\index{risk measures}`{=latex} encompass the set of functions that map a set of outcomes to an output value characterizing the associated riskiness of those outcomes. As is usual when attempting to compress information (e.g., condensing information into a single value), there are multiple ways we can characterize this riskiness.

##### Coherence & Other Desirable Properties

Further, it is desirable that a risk measure has certain properties, and risk measures that meet the first four criteria are called "coherent" in the literature. In "An Introduction to Risk Measures for Actuarial Applications" [@hardy2006introduction], Hardy describes four properties. Using $H$ as a risk measure and $X$ as the associated risk distribution:
...
The authors of "Properties of Distortion Risk Measures" [@balbas2009properties] also note other properties of interest:
...
2.  $\lim_{u\to0^+} g'(u) = \infty$ and $\lim_{u\to1^-} g'(u) = 0$.
...
|---------------|---------------|---------------|---------------|---------------|
| VaR`\index{Value at Risk (VaR)}`{=latex}                          | No       | No       | No         | No        |
| CTE`\index{Conditional Tail Expectation (CTE)}`{=latex}                          | Yes      | No       | No         | No        |
| DualPower $(y > 1)$          | Yes      | Yes      | Yes        | No        |
| ProportionalHazard $(γ > 1)$ | Yes      | Yes      | Yes        | No        |
...
Note that $F^*$ and $F$ are equivalent probability measures if and only if $g:[0,1]\to[0,1]$ is continuous and one-to-one. We define a family of distortion risk-measures using the mean-value under the distorted probability $F^*(x)=g(F(x))$:
...
We will assume a distribution of outcomes and show how the risk measures behave across a range of levels $\alpha$:
...
### Variance Reduction Techniques`\index{variance reduction}`{=latex}
...
A control variate`\index{control variates}`{=latex} is a random variable whose expected value is already known (or can be estimated with high precision). By simulating the main process alongside the control variate and leveraging the differences in outcomes, the overall variance of the estimator is reduced. In finance, for instance, one might use a simpler instrument whose price is analytically known as a control variate when simulating a more complex derivative.

#### Antithetic Variates

In antithetic variates`\index{antithetic variates}`{=latex}, each random draw ($X$) is paired with a corresponding draw ($-X$) or another symmetric transformation. The idea is that positive and negative deviations tend to cancel out, thereby reducing overall variability. For example, if simulating geometric Brownian motion paths, each path with a particular series of random shocks could be matched with a path using the same shocks negated. Updating estimates with the average result of these pairs generally yields a lower variance for the same number of total draws.

#### Stratified Sampling

In stratified sampling`\index{stratified sampling}`{=latex}, instead of sampling purely at random across the entire probability distribution, the distribution is divided into "strata" (or sub-intervals), and draws are forced to fall into each of these strata with a pre-specified frequency. By covering the distribution more systematically, the resulting estimates often converge faster. A related approach is **quasi-Monte Carlo**`\index{quasi-Monte Carlo}`{=latex}, which uses deterministic low-discrepancy sequences (such as Sobol or Halton sequences) to span the sample space more uniformly than random sampling. Julia has a package for this called [QuasiMonteCarlo.jl](https://github.com/SciML/QuasiMonteCarlo.jl).
...
# Random number generator (Xoshiro as default with a seed to ensure reproducibility)
rng_gen = Xoshiro(1234)

# Function to simulate the model
function simulate_stochastic_solow(rng, T, α, δ, s, n, g, σ, K0, A0)
    # Shock distribution (normal distribution for productivity shocks)
    shock_distribution = Normal(0, σ)
    K = zeros(T)        # Capital (per effective worker) over time
    Y = zeros(T)        # Output (per effective worker) over time
    ...
    for t in 1:(T-1)
        ...
        # Population and technology growth dilute capital per effective worker
        K[t+1] /= (1 + n) * (1 + g)
    end
...
# Simulate the model
K, Y, A = simulate_stochastic_solow(rng_gen, T, α, δ, s, n, g, σ, K0, A0)
...
Modern computers use pseudo-random number generators (PRNGs)`\index{pseudo-random number generator (PRNG)}`{=latex}—algorithms that produce sequences of numbers that appear random but are actually determined by an initial seed. Given the same seed, you get the same sequence. PRNGs also have a finite period: after enough generated values, the sequence repeats. Choose PRNGs with periods that are long enough for your application.
