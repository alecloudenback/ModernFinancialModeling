...
Counterintuitively, Julia --- despite having a far smaller training corpus than Python --- consistently performs as well or better in LLM code generation benchmarks. In a study evaluating ChatGPT 3.5 across 10 programming languages, Julia achieved the highest success rate at 81.5%, while C++ had the lowest at 7.3% [@buscemi2023codegen]. The MultiPL-T project, which fine-tuned open-source Code LLMs on low-resource languages including Julia, found that these models outperformed other open Code LLMs on the MultiPL-E benchmark.
