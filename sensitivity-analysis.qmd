...
Sensitivity analysis`\index{sensitivity analysis}`{=latex} tells us how fragile our valuation or risk metrics are once key drivers shift. We will move from **local techniques** (derivatives, finite differences, automatic differentiation) that explain the effect of tiny shocks to **global techniques** (regression-based screening, Sobol, Morris, FAST) that summarize how entire distributions of inputs ripple through a model. The chapter closes with practical scenario-analysis guidance so you can turn the math into board-ready stress narratives.
