...
Sensitivity analysis`\index{sensitivity analysis}`{=latex} tells us how fragile our valuation or risk metrics are once key drivers shift. We will move from **local techniques**`\index{sensitivity analysis!local methods}`{=latex} (derivatives, finite differences, automatic differentiation) that explain the effect of tiny shocks to **global techniques**`\index{sensitivity analysis!global methods}`{=latex} (regression-based screening, Sobol, Morris, FAST) that summarize how entire distributions of inputs ripple through a model. The chapter closes with practical scenario-analysis guidance so you can turn the math into board-ready stress narratives.
...
Given a basic insurance product, a pure whole of life (WOL) policy with level benefits and level premiums payable within the first 10 years, the reserve in-force at the end of the $y^{th}$ ($y \geq 1$) policy year is defined by

$$
res(y) = \sum_{t=age+y}^{120} \frac{sur_{t-age}}{sur_y} \left( mort_t * B / \sqrt{1 + pir} - P_t \right)
$$

where

- $mort_t$ is the mortality rate at attained age $t$
- $sur_x$ is the survival probability from issue, discounted with the policy interest rate, with values of
    + $sur_0 = 1$,
    + $sur_x = sur_{x-1} * (1 - mort_{age+x-1}) / (1 + pir)$ for $x \geq 1$, and
    + 0 once $age + x$ reaches 120, the ultimate age of the mortality table
- dividing by $sur_y$ conditions on survival to the valuation date and removes the discounting for the first $y$ years, so cashflows are valued per policy in force at time $y$
- $B$ is the level benefit throughout the policy
- $P_t$ is the level premium, payable at the start of each of the first $pp$ (here 10) policy years — that is, while $t - age < pp$ — and 0 afterward
...
function res(y::Int, pol::Policy; Ω=120)
    ...
    if y >= 1 && y < Ω - pol.issue_age
        ...
        for t in (pol.issue_age+y):Ω
            prem = 0.0
            if t - pol.issue_age < pol.pp
                prem = pol.prem
            end
            s += sur(t - pol.issue_age, pol) / sy *
                 (mort[pol.sex][t] * pol.benefit_base / sqrt(1 + pol.pir) - prem)
        end
    end
    s
end
...
Central finite differences`\index{finite differences}`{=latex} approximate derivatives by shocking one input up and down while holding everything else fixed. The result can be interpreted as a "delta" (absolute change per unit of input) or, if we scale by the base reserve as in the code below, as a *semi-elasticity*: the proportional change in the reserve per unit change in the input, $\frac{1}{V}\frac{\partial V}{\partial r}$. (A true elasticity would additionally scale by the base input level, $\frac{r}{V}\frac{\partial V}{\partial r}$, giving the percentage change in output per percentage change in input.)
...
# semi-elasticity: proportional change in the year-2 reserve
# per unit change in the interest rate, evaluated at r = 3%
# with a 10-basis-point (h = 0.001) perturbation
...
Regression-based sensitivity (also called standardized regression coefficients) fits a metamodel between sampled inputs and the resulting reserves, then reports correlations and partial correlations as quick importance scores.`\index{GlobalSensitivity.jl}`{=latex}
...
Sobol indices`\index{Sobol indices}`{=latex} are a variance-based method that decomposes the variance of the output of the model or system into fractions which can be attributed to inputs or sets of inputs. This helps to get not just the individual parameter's sensitivities, but also gives a way to quantify the effect and sensitivity from the interaction between the parameters.
...
::: {.callout-note}
## Randomized quasi-Monte Carlo for design matrices
...
The Morris method`\index{Morris method}`{=latex}, also known as Morris's OAT method (where OAT stands for One At a Time), can be described in the following steps:
...
FAST (Fourier Amplitude Sensitivity Tests)`\index{FAST (Fourier Amplitude Sensitivity Tests)}`{=latex} offers a robust and computationally efficient procedure, especially at low sample sizes, to get the first and total order indices as discussed in Sobol. It utilizes monodimensional Fourier decomposition along a curve exploring the parameter space. Each input factor is varied along the curve via a parametric equation,

$$
x_i(s) = G_i(\sin(\omega_i s))
$$

where $s$ is a scalar variable varying over the range $-\pi < s < \pi$, $G_i$ are transformation functions and $\omega_i, \forall i=1,2,\ldots,N$ is a set of different (angular) frequencies, to be properly selected, associated with each factor.

```{julia}
using GlobalSensitivity

# reserve @ year 1/2, interest rate ∈ [0.01, 0.05], prem ∈ [500, 1500]
fast = gsa(r1_wrt_r, eFAST(), [[1, 2], [0.01, 0.05], [500, 1500]], samples=1000)
println(fast.S1)
println(fast.ST)
```
...
Reverse stress tests`\index{reverse stress testing}`{=latex} start from an unacceptable outcome (e.g., statutory RBC < 250%) and work backward to the combination of market, actuarial, and operational events that would trigger it.
