...
Central finite differences`\index{finite differences}`{=latex} approximate derivatives by shocking one input up and down while holding everything else fixed. The result can be interpreted as a "delta" (absolute change per unit of input) or as an elasticity if we scale by the base reserve.
...
# percentage change in reserve at year 2 when
# the interest rate is at 3% with a perturbation of 0.1%
