...
- $p_y$ is the survival probability adjusted with the policy interest rate, with values of
    + $p_{y-1} = 1$,
    + $p_x = p_{x-1} * (1 - mort_{age+y}) / (1 + pir)$ for $x >= y$, and
...
function sur(y::Int, pol::Policy)
    ...
    elseif y < 0 || 120 - y <= pol.issue_age
        ...
        sur(y - 1, pol) * (1 - mort[pol.sex][pol.issue_age+y]) / (1 + pol.pir)
    end
end
...
Define a customized finite difference function with respect to the interest rate, rippled by a small difference.
...
@show reg_anal.spearman
```

The Pearson and Spearman coefficients show the correlation coefficient matrix between inputs and outputs.
...
Scenarios can be generated following scenario generation methodologies to evaluate impacts. More details in @sec-scenario-generation on scenario generation.
