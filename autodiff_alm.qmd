...
# Automatic Differentiation for Asset Liability Management

```{julia}
#| echo: false
#| output: false
using Pkg
Pkg.activate("env/autodiff_ALM")
Pkg.instantiate()
```

## Chapter Overview

Asset liability modeling`\index{asset liability management (ALM)}`{=latex} requires computing derivatives of portfolio values with respect to yield curve changes. Traditional approaches use finite difference methods or analytical approximations, but automatic differentiation ("autodiff" or "AD") provides exact derivatives with minimal additional computation. This chapter demonstrates how to implement ALM workflows using autodiff`\index{DifferentiationInterface.jl}`{=latex} in Julia.
...
We start by constructing a yield curve`\index{yield curve}`{=latex} using cubic spline interpolation:
...
The `(b::FixedBond)(curve)` function (a **callable struct**`\index{callable struct}`{=latex}, since we are using the `b` object itself as the function invocation) takes the curve and returns an array of `Cashflow`s.
...
Rather than approximating derivatives through finite differences, autodiff computes exact values, gradients, and Hessians. The concepts and background are covered in @sec-autodiff.
...
The second element of `vgh_liab` is the partial derivative with respect to each of the inputs (here, just the `zero_rates` that dictate the curve). The sum of the partials is the (dollar) sensitivity of the liabilities to a parallel shift in rates—a dollar duration, which becomes the effective duration`\index{duration}`{=latex}`\index{duration!effective}`{=latex} once divided by the value (shown below).
...
This matrix captures the convexity`\index{convexity}`{=latex} characteristics of the liability portfolio. The diagonal elements represent "key rate convexities"`\index{key rate duration}`{=latex}`\index{duration!key rate}`{=latex}—how much the duration at each key rate changes as that specific rate moves:
...
# Arguments
- `N::Int`: # of loop iterations to unroll (must be a compile-time constant)
- `for_loop`: A standard for loop expression

"""
macro unroll(N::Int, loop)
    Base.isexpr(loop, :for) || error("only works on for loops")
    Base.isexpr(loop.args[1], :(=)) || error("Loop pattern not supported")
...
[^zulip]: With thanks to the helpful people on the Julia Zulip and in particular Mason Protter for this approach. 
