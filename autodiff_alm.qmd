...
using DifferentiationInterface: AutoForwardDiff, gradient, hessian, value_gradient_and_hessian
...
function optimize_portfolio(assets, vgh_assets, liabs, vgh_liabs, constraints)
    ...
    @constraint(model, sum(w[i] * vgh_assets[i][1] for i in 1:n) + vgh_liabs[1] == 0)
    ...
    for j in 1:length(vgh_liabs[2])
        gradient_gap = sum(w[i] * vgh_assets[i][2][j] for i in 1:n) + vgh_liabs[2][j]
