...
The second element of `vgh_liab` is the partial derivative with respect to each of the inputs (here, just the `zero_rates` that dictate the curve). The sum of the partials is the effective duration`\index{duration}`{=latex}`\index{duration!effective}`{=latex} of the liabilities. 
