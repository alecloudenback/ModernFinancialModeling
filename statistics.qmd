...
We can characterize the entire distribution $X$ via the **entropy**`\index{entropy}`{=latex}, $H(X)$, of a probability set is the ensemble's average information content:
...
An outcome is additive and results in a normal distribution if it's the sum or difference of multiple independent processes. Examples include applications of the **Central Limit Theorem**`\index{Central Limit Theorem}`{=latex}. Examples of this include:
...
To compare the likelihoods of the two models, we can calculate the ratio of their posterior probabilities, known as the **Bayes factor**`\index{Bayes factor}`{=latex}, $BF$:
