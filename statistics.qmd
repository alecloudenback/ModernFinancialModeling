...
In reality, we don't observe the parameters we are interest in determining the values for. 
...
current_prob = sum(logpdf(Normal(μ_current, σ_current), r) for r in returns) + logpdf(μ_prior, μ_current) + logpdf(σ_prior, σ_current)
...
```{julia}
#| label: fig-MH-MCMC
#| fig-cap: "The blue lines of the MCMC chain explore the posterior density of interest (after discarding the burn-in samples in red)."

# Plot the chain
let 
    f = Figure()
    ax1 = Axis(f[1, 1],yreversed=true)

    scatterlines!(ax1,chain[1:burn_in,1],1:burn_in,color=(:red,0.5),markercolor=:red)
    scatterlines!(ax1,chain[burn_in+1:end,1],burn_in+1:num_samples,color=(:blue,0.5),markercolor=:blue)

    ax2 = Axis(f[2,1],xlabel="μ")

    hist!(ax2,chain[burn_in+1:end,1],color=:blue)
    
    linkxaxes!(ax1, ax2)

    ax3 = Axis(f[1, 2],yreversed=true)

    scatterlines!(ax3,chain[1:burn_in,2],1:burn_in,color=(:red,0.5),markercolor=:red)
    scatterlines!(ax3,chain[burn_in+1:end,2],burn_in+1:num_samples,color=(:blue,0.5),markercolor=:blue)

    ax4 = Axis(f[2,2],xlabel="σ")
    hist!(ax4,chain[burn_in+1:end,2],color=:blue)
...
This was a very quick introduction to MCMC sampling, with the point being that the technique is not magic. More advanced libraries will perform the sampling for you with more advanced algorithms than Metropolis-Hastings, but by demonstrating that we could do-it-from-scratch in a small amount of code is intended to make the approach less intimidating.
...
We will construct a Bayesian model using the Turing.jl library which allows us to step back from the intricacies of defining our own sampler and routine. The goal is to fit the parameters of one of the competing models from above in order to demonstrate an MCMC analysis workflow and essential concepts.
