...
Probability, statistics, machine learning, signal processing, and even physics have a foundational link in **information theory**`\index{information theory}`{=latex}, which is the description and analysis of how much useful data is contained within a signal or dataset. We will work through this with concrete examples.
...
$$
H(X) = \sum_{i} p(x_i)\,\text{log}_2\frac{1}{p(x_i)} = -\sum_{i} p(x_i)\,\text{log}_2 p(x_i)
$$ {#eq-entropy}
...
With the uniform prior assumption, $H(X) = 3.322 \text{ bits}$ and $h(x_i)$ is also uniform. Note that $H$ is higher for the uniform prior than the prior in @tbl-digit-information-human. We will not prove it here, but a uniform probability over a set of outcomes is the highest entropy distribution that can be assumed for this problem. A higher entropy prior can be viewed as less biased in a specific sense: among all distributions consistent with the constraints we have explicitly stated (here, only that the missing digit is one of ten values), the maximum entropy distribution encodes the fewest additional assumptions. This is known as the **maximum entropy principle**`\index{maximum entropy principle}`{=latex}. It does not mean that higher entropy priors are automatically better — our round-number prior deliberately encodes real knowledge about human behavior, which is exactly what a good prior should do.
...
We could repeat the analysis to determine the information gain if we were to split the tree based on having good credit. However, we can already conclude that `employed` is the better attribute to split the data on. Splitting on `good_credit` produces two subsets that each have the same proportion of defaults (3 out of 4) as the overall data, so the weighted entropy after that split equals `H0` and the information gain is zero. In contrast, `IG_employment` (`{julia} round(IG_employment;digits=3)`) recovers a majority of the overall entropy `H0` (`{julia} round(H0;digits=3)`). This also matches our intuition when looking at @tbl-default-data as the eye can spot a higher correlation between `employed` and `default` than `good_credit` and `default`.
...
- **Normal distribution**`\index{Normal distribution}`{=latex} when the mean and variance are known (fixed) and outcomes range over the entire real line, but the distribution is otherwise unconstrained.
- **Exponential distribution**`\index{Exponential distribution}`{=latex} when outcomes lie in $[0,\infty)$ and we know only the mean.
...
Additional maxent distributions and associated constraints are listed in @tbl-MEDs. Those distributions arise again and again in nature because of the second law of thermodynamics - nature likes to have constantly increasing entropy and therefore it should be no surprise that (random) processes that maximize entropy pop up all over the place. The **second law of thermodynamics** in physics is an analogy: it states that a closed system tends to move toward higher entropy states. Similarly, in purely probabilistic settings, when few constraints are imposed, the system’s “natural” distribution tends to be the one that maximizes entropy.
...
| Bounded range (0 to 1) with fixed mean of $\ln(x)$ and of $\ln(1-x)$ |  | Beta |
| Fixed number of independent binary trials with known mean | Binomial |  |
| Fixed (known) mean, non-negative outcomes | Geometric | Exponential |
| Fixed mean and fixed mean of logarithm, range is \> zero |  | Gamma |
| Fixed mean and variance |  | Gaussian (Normal) |
| Fixed mean count of events across many independent, individually rare trials | Poisson |  |

: Maximum Entropy Distributions and the conditions under which they are applicable. For example, if you know that a quantity is continuous and non-negative and you know its mean, then the MED is the Exponential Distribution. (Strictly, the Poisson is the maximum-entropy distribution among sums of independent binary events with a given mean; with only a fixed mean on the non-negative integers, the Geometric applies.) {#tbl-MEDs tbl-colwidths="[50,25,25]"}
...
|------------------------|------------------------|------------------------|
| Many *additive* pluses and minuses that move an outcome in one dimension | Normal | Sum of many dice rolls, errors in measurements, sample means (Central Limit Theorem) |
| Many *multiplicative* pluses and minuses that move an outcome in one dimension | Log-normal | Incomes, sizes of cities, stock prices |
...
: Underlying processes create typical probability distributions. That there is significant overlap with the distributions in @sec-maxent is not a coincidence. {#tbl-prob-processes tbl-colwidths="[44,22,34]"}
...
[^statistics-4]: Multiplicative processes are often referred to as "geometric", as in "geometric Brownian motion" or "geometric mean". Additive processes are sometimes referred to as "arithmetic". The root of this confusing terminology appears to be due to the fact that series involving repeated multiplication were solved via geometric (triangles, angles, etc.) methods while those using sums and differences were solved via arithmetic.
...
We will show the likelihood of the three models after deriving the **maximum likelihood estimate (MLE)**`\index{maximum likelihood estimation (MLE)}`{=latex}, which is simply found by determining the parameters that maximize the calculated likelihood. In general, this can be accomplished by an optimization routine, but here we will just use the functions built into Distributions.jl:
...
```{julia}
#| label: fig-rain-cdf
#| fig-cap: "Empirical and fitted CDFs for rainfall data."
let x = rain
...
We evaluated the likelihood at a single point estimate of the parameters, but a true posterior probability of the parameters of the distributions will be represented by a *distribution* rather than a point. The rest of the chapter will describe how to express the posterior probabilities of the parameters for $H_1$, $H_2$, and $H_3$ using Bayesian statistical methods.
...
The FCF is the belief that if you have an $X\%$ confidence interval, the probability that the true value of interest lies within that interval is $X\%$. This is false, and an abbreviated explanation of why is as follows:
...
-   However, once the data is observed, the probability that the true value lies within the realized interval need no longer be $X\%$.

Here's an example taken from the article referenced above: we wish to estimate the mean of a continuous random variable. We observe two data points $y_1$ and $y_2$. If $y_1 < y_2$ then we say our interval is $(-\infty,\infty)$ otherwise the interval is empty. Our confidence procedure creates an interval for which 50% of the time it contains the true value. However, once we've observed the data and created the interval, then "post-data" we can tell with certainty whether our interval contains the variable of interest. We can only say "pre-data", or pre-observations, that the confidence interval is really $X\%$ probable to contain the right value. Similar (and other) issues arise with more "real world" examples of confidence intervals.
...
Another meaningful way to understand the contrast between Bayesian and Frequentist approaches is through the lens of decision theory, specifically how each view treats the concept of randomness. This perspective pertains to whether you regard the data as being random or the parameters as being random.
...
The resulting chain contains a list of points that the algorithm has moved along during the sampling process. Note that there is a `burn_in` parameter. This is because we want the chain to iterate long enough that (1) the samples are effectively independent of the arbitrary starting point, and (2) different chains are effectively independent of one another.
...
The issues above mean that MCMC sampling is very computationally expensive for more complex examples. Compared with Metropolis-Hastings, modern algorithms (such as the No-U-Turn Sampler (NUTS)`\index{NUTS (No-U-Turn Sampler)}`{=latex}) explore the posterior distribution more efficiently by avoiding back-tracking to already explored regions and dynamically adjusting the proposals to adaptively fit the posterior. Many of them take direct influence from particle physics, with the algorithm keeping track of the energy of the current state as it explores the posterior space.

Algorithms have only brought so much relief to the modeler with finite resources and compute. There is still a lot of responsibility for the modeler to design models that are computationally efficient, transformed to eliminate oddly-shaped density regions, or find the right simplifications to the analysis in order to make the problem tractable.
...
2.  We presume that there will be positive rainfall and that about 95% of the prior probability for median annual rainfall ($e^\mu$) falls between $exp(2)$ and $exp(6)$, or about 7 and 403 inches.
3.  Distributions.jl's `Exponential(θ)` is parameterized by its mean, so this prior on σ has a mean of 2 — a diffuse choice, since in a LogNormal model even σ of 0.5 covers a lot of variation in outcomes.
...
An `ess` greater than the number of samples indicates that the chain exhibited negative autocorrelation (each draw contributes more information than an independent sample would). An `ess` much less than the number of samples indicates that the chain isn't sampling very efficiently but, aside from needing to run more samples, isn't necessarily a problem.
...
```{julia}
#| label: fig-rain-mcmc
#| fig-cap: "The prior model shows a wide range of possible outcomes, and the shape of the distribution is reasonable: there's a nice 'S' shape to the CDF, indicating a dense region where most outcomes would fall in the PDF. The fitted posterior model (bottom) has good coverage of the observed data (shown in blue)."
