...
An expert knowledgable in relevant domains is crucial. Imagine if someone said, "Let's emulate an architect by having a construction worker and an artist work together." It's all too common for businesses to pair a business expert with an IT person and hope for the finished product to be as good as one crafted by someone skilled in all the right domains.
...
### Model Theory
An essential part of financial modeling involves building up of modeler's expertise. Then, we must characterize that knowledge more explicitly. 
...
- is keenly aware of the small world versus large world distincion (@sec-small-large-world)

[^financial-modeling-practice-2]: The idea of "model theory" is adapted from Peter Naur's 1985 essay, "Programming as Theory Building". 
...
The world is complicated in ways we can sometimes describe but never fully anticipate. A humble modeler tries to understand what the model can and cannot claim. And in good faith will try to share the limitations the model has with stakeholders, such as saying something like "we have a lot of data for low-rate environments but rapidly rising environments haven't been observed in the dataset".
...
11. Implementation and maintenance plan: Who owns the model once initial objectives are met?
...
If the soul of the model is its logic, then the lifeblood is its data. In practice, a model's fate is often sealed not by the sophistication of its algorithms, but by the quality of the data it consumes. Even the most elegant model is helpless in the face of stale, sloppy, or misunderstood inputs.
