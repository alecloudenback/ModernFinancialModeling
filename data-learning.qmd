...
EDA helps discover patterns, relationships, and insights within the data. One can perform the following analyses, though this is not an exhaustive list:
...
- Supervised Learning`\index{supervised learning}`{=latex} (with labeled data):
...
- Unsupervised Learning`\index{unsupervised learning}`{=latex} (without labeled data):
    + Clustering: K-means, DBSCAN, hierarchical clustering.
    + Dimensionality Reduction: PCA, t-SNE, or UMAP.
- Reinforcement Learning`\index{reinforcement learning}`{=latex}: Learn from interactions with an environment (e.g., Q-learning, Deep Q-Networks).

### Train and evaluate the model

- Split the data: Use either a train-test split (e.g., 80/20 or 70/30 split) or cross-validation`\index{cross-validation}`{=latex} (e.g., k-fold cross-validation).
- Fit the model: Train the model on the training set.
- Evaluate the model: Use evaluation metrics appropriate to the task.

### Tune hyperparameters

Hyperparameters`\index{hyperparameters}`{=latex} control how models learn. One can use techniques like the following to tune hyperparameters:
...
- Models may be overfitting`\index{overfitting}`{=latex} or underfitting`\index{underfitting}`{=latex}. Overfitting occurs when a model is too complex and captures noise instead of the underlying pattern, leading to poor generalization to new data. Underfitting occurs when the model is too simple to capture the relevant structure in the data.
...
Forecasting`\index{forecasting}`{=latex} is the process of making predictions about future events or outcomes based on historical data, patterns, and trends. It involves the use of statistical methods, machine learning models, or expert judgment to estimate future values in a time series or predict the likelihood of specific events. Forecasting is widely used in fields like economics, finance, meteorology, supply chain management, and business planning.

Here is an example how to do time series forecasting in Julia, where a shaded band shows the 95% confidence interval derived from the forecast covariance:
