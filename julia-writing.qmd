...
This is the standard (Julia) mode of the REPL, but there are three other modes you need to know. Each mode is entered by typing a specific character after the `julia>` prompt. Package mode persists between commands, while help and shell modes return you to the `julia>` prompt after a single command. To exit a mode manually, hit backspace after the prompt and you'll get the `julia>` prompt back.
...
Notebooks are a popular alternative to IDEs when it comes to short and self-contained code, typically in data science. They are also a good fit for literate programming`\index{literate programming}`{=latex}, where lines of code are interspersed with comments and explanations.
...
::: callout-note
TOML (Tom's Obvious, Minimal Language) is a modern configuration file format used to store settings and data in a human-readable, plaintext format.
:::
...
::: callout-note
Starting with Julia 1.11, Julia will preferentially use version-specific manifest files such as `Manifest-v1.11.toml` if present, making it nicer to work with multiple Julia versions at one time on a single system.
:::
...
For guidance on making a sysimage, see [PackageCompiler.jl](https://github.com/JuliaLang/PackageCompiler.jl) or the [Julia VS Code extension's features](https://www.julia-vscode.org/docs/v1.0/userguide/compilesysimage/) that assist in their creation.
:::
...
``` julia
using PkgTemplates
cd(expanduser("~/.julia/dev")) # `cd` does not expand `~` on its own
...
To run tests in Julia, activate the package's environment (e.g. `]activate .` from the package directory) and run `Pkg.test()`, or use the `]test` command in the Julia REPL. This will run the file `test/runtests.jl` in the package directory. The test infrastructure automatically handles setting up the correct environment and dependencies for testing. Test coverage reports can be generated to see which lines of code are exercised by your tests.
