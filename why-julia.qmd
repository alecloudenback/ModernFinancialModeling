...
Julia is relatively new[^why-julia-1], and *it shows*. It is evident in its pragmatic, productivity-focused design choices, pleasant syntax, rich ecosystem, thriving communities, and its ability to be both very general purpose and power cutting edge computing.
...
This is a really simple example relating `Cession`s, `Policy`s, and `Live`s to do simple retention analysis. Retention is a measure of how much risk an insurance company holds on a policy after it's own reinsurance risk transfer ("ceded amounts").
...
We find in @tbl-bsm-benchmark that despite the syntatic similarity, Julia is much faster than the other two.
