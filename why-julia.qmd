...
We defined `retained` and adapted it to mean related but different things depending on context. We didn't have to define `retained_life(...)` and `retained_pol(...)` because Julia can **dispatch**`\index{multiple dispatch}`{=latex} based on what you give it (this is a more powerful, generalized version of method dispatch commonly used in object-oriented programming; see @sec-data-types for more).
