...
We will use a toy three-asset portfolio throughout the chapter. The vector μ contains annualised expected returns and Σ stores the covariance matrix implied by historical or modeled volatilities. Keeping the dataset intentionally small lets us focus on the optimisation mechanics before scaling up to tens or hundreds of assets in practice.
...
@objective(model, Max, (dot(μ, w) - rfr) / sqrt(sum(w[i] * Σ[i, j] * w[j] for i in 1:n_assets, j in 1:n_assets) + eps()))
# Constraints: Sum of portfolio weights should equal to 1, and all weights should be zero or positive
@constraint(model, sum(w) == 1.0)
# Solve the optimization problem
optimize!(model)
...
- Always start with clean inputs: align expected returns, covariances, and capital charges to the same horizon and currency before optimising.
