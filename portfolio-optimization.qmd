...
Harry Markowitz introduced modern portfolio theory`\index{modern portfolio theory}`{=latex} in 1952. The main idea is that investors seek to maximize the expected return of a portfolio given a certain amount of risk. Under standard assumptions, achieving higher expected returns typically requires accepting higher risk; portfolios that simultaneously deliver higher return and lower risk dominate others and define the upper boundary of the feasible set. The efficient frontier traces the best achievable tradeoff.
...
\text{subject to} \quad &{\mu}^{T}{w}\geq{\mu}_{\text{target}}\\
&{1}^{T}{w}={1}\\
&{w}\geq{0}
\end{aligned}
$$
...
# Set up weights as variables to optimize
@variable(model, w[1:n_assets] >= 0.0)
# Objective: maximize mean-variance utility under posterior returns
...
\text{where} \quad
RC_i = \frac{w_i (\Sigma w)_i}{w^{T} \Sigma w + \epsilon}, \quad t = \frac{1}{N}
\end{aligned}
$$
...
@expression(model, target_contribution, 1 / n_assets)
...
\text{subject to} \quad & \sum_{i=1}^{N} w_i = 1 \\
& w_i \geq 0, \quad \forall i \\
& (w - w_0)^{T} \Sigma (w - w_0) \leq \epsilon
\end{aligned}
$$
