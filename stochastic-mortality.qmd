...
This would be a totally valid and logical approach! However, high performance is a top priority for this simulation, and an abstractly typed field carries a cost. A `sex::Sex` field could hold *any* subtype of `Sex` — one as simple as our definitions above (with no fields in `Male` and `Female`), or a new `Alien` subtype someone defines later with a number of associated data fields! Because Julia cannot determine a fixed inline representation covering every possible subtype of `Sex`, the field must be stored as a reference rather than as plain data. Consequently, `Policy` is no longer an `isbits` type (a property discussed below): an array of policies still stores its `Policy` elements contiguously, but each element carries a reference where the enum-based version below carries the value itself, and code reading the field may require runtime type checks or dynamic dispatch. The result is a less compact and less compiler-friendly layout.
...
::: callout-note
Type annotations are optional, but providing them is able to coerce the values to be all plain bits (i.e. simple, non-referenced values—unlike arrays, which are heap-allocated and accessed by reference) when the type is constructed. We could have defined `Policy` without the types specified:
...
We would also find that the un-annotated type is many times slower (roughly 20 times slower when benchmarked on the hardware used to render this book) than the one with annotation due to the need to utilize runtime lookup and reference memory on the heap instead of the stack.
:::
...
function pol_project!(out, policy, params)
    ...
    @inbounds for t in 1:min(params.proj_length, ω - start_age + 1) # <1>
        ...
        if (rand() < q)
            ...
            # survived: add a life to the output count
...
1.  `inbounds` turns off bounds-checking, which makes hot loops faster, but first write the loop without it to ensure you don't have an out-of-bounds bug (with bounds-checking disabled, an out-of-bounds access is undefined behavior and may crash or silently corrupt results).
2.  The death benefit is accumulated into year `t`'s bucket and the function returns, so a dead life adds no further lives or benefits to the projection.
