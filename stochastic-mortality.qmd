...
Enums are convienent because it lets us use human-meaningful names for integer-based categories. Julia will also keep track of valid options: we cannot now use anything other than `Female` or `Male` where we have said a `Sex` must be specified.

:::callout-note
There exist Julia packages which are more powerful versions of Enums, esstially leaning into the ability to use the type system instead of just nice names for categorical variables.
:::
...
```{julia}
let
    p = Policy(1, Male, 1e6, 0.02, 1, today(), 50, Standard)
...
We would also find that the un-annotated type is about 50 times slwoer than the one with annotation due to the need to utilize runtime lookup and reference memory on the heap instead of the stack.
:::

### The Data

To partially illustrate a common workflow, we'll pretend that the data we are interested in comes from a CSV file, which will be defined inline using an `IOBuffer` so that the strucutre of the source data is clear to the reader. Only two policies will be listed for brevity, but we will duplicate them for simulation purposes later on.
...
2. Step through annual timesteps and simulate whether a death has occured. 
    1. If a death has occured, log the benefit paid out.
    2. If a death has not occured keep track of the remaining lives inforce and increment policy values.
...
1. `vec` turns the result into a 1D vector instead of a 1D matrix for later convienence.
...
This defines a loop to calculate the reults `n` times (this is only running the two policies in the sample data `n` times). This is emulating running our population of policies through $n$ stochastic scenarios, similar to what might be done for a risk or pricing exercise.
...
Using a 2022 Macbook Air M3 laptop, about 45 million policies able to be stochastically projected per second:
...
This example has worked through a recommended pattern of setting up and running a stochstic simulation using a threaded approach to parallelism. The results show that quite a bit of simulation power is avaiable using even consumer laptop hadware!