...
This would be a totally valid and logical approach! However, high performance is a top priority for this simulation, and an abstractly typed field carries a cost. A `sex::Sex` field could hold *any* subtype of `Sex` — one as simple as our definitions above (with no fields in `Male` and `Female`), or a new `Alien` subtype someone defines later with a number of associated data fields! Because Julia cannot assume that subtypes of `Sex` will always be simple singletons with no associated data, it cannot store the value inline within `Policy`: the field instead holds a reference to a separately allocated object, `Policy` loses the `isbits` property discussed below, an array of policies becomes an array of pointers rather than one compact block of memory, and code reading the field must look up the concrete type at runtime (dynamic dispatch).
