...
The simulation uses a threaded parallelism approach with `ChunkSplitters`\index{ChunkSplitters.jl}`{=latex}` to divide work across available CPU threads. This pattern ensures thread safety\index{thread safety}`{=latex}` by:
...
1. The discount rate (5%) differs from the COLA (3%), so the timing of death affects the present value. Earlier deaths produce higher present values since benefits haven't grown as much but also haven't been discounted as heavily.

## Benchmarking

Many millions of policies able to be stochastically projected per second using a laptop computer[^stochastic-mortality-1]:
