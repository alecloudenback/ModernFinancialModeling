...
::: callout-note
Thread jargon varies. A compact glossary helps:
...
1.  Compute a chunk size that covers the full range even when `a` is not perfectly divisible by the thread count (`cld` is ceiling division).
2.  Build a generator of `UnitRange`s that partition `1:a` without dropping any trailing values.
3.  Launch a task per chunk; each task computes a single-threaded sum for its range.
4.  Collect the partial sums when the tasks finish.
5.  Perform one final single-threaded aggregation. This last step is cheap because there are only as many partial sums as threads.
...
| Google | Tensor processors | XLA (via TensorFlow or JAX) |
...
[OhMyThreads.jl](https://github.com/JuliaFolds2/OhMyThreads.jl) provides the threaded versions of essential functions such as `tmap`, `tmapreduce`,`tcollect`, and `tforeach` (see @tbl-funcional-methods). In most cases, the chunking and data sharing is handled automatically for you.
