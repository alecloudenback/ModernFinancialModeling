...
| `raw"..."` | A string literal prefix that disables string interpolation and escape sequences. Useful for writing raw string data. This is especially helpful when working with filepaths where the `\` in Windows paths otherwise needs to be escaped with a leading slash (e.g. `\\` ). |
