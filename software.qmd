...
**Test Driven Development** (TDD)`\index{test-driven development (TDD)}`{=latex} is a software development approach where tests are written before the actual code. The process typically follows these steps:
...
By thinking through what the correct result for those different cases is, we have forced ourselves to think about how the function should work generically:
...
The good news is that we can! **Test coverage**`\index{test coverage}`{=latex} is a measurement related to how much of the codebase is covered by at least one associated test case. In the following example, code coverage would flag that the `... other logic` is not covered by tests and therefore encourage the developer to write tests covering that case:
...
Test coverage is also a great addition when making modifications to code. It can be set up such that you receive reports on how the test coverage changes if you were to make a certain modification to a codebase. An example might look like this for a proposed change which added 13 lines of code, of which only 11 of those lines were tested ("hit"). The total coverage percent has therefore gone down (`-0.49%`) because the proportion of new lines covered by tests is $11/13 \approx 85\%$, which is lower than the original coverage rate of 90%.
...
**Docstrings**`\index{docstrings}`{=latex} (documentation strings) are intended to be a user-facing reference and help text. In Julia, docstrings are just strings placed in front of definitions. Markdown[^software-1] is available (and encouraged) to add formatting within the docstring.
...
Docstrings become even more valuable when you hook them into automated documentation generators such as [Documenter.jl](https://github.com/JuliaDocs/Documenter.jl). Documenter ingests Markdown pages plus in-code docstrings, renders math, runs doctests, and publishes a searchable site every time CI runs. Writing rich docstrings up front means your docsite, help mode, and notebook snippets stay perfectly in sync.

### Docsites

**Docsites**`\index{docsites}`{=latex}, or documentation sites, are websites that are generated to host documentation related to a project. Modern tooling associated with programming projects can generate a really rich set of interactive documentation while the developer/modeler focuses on simple documentation artifacts.
...
-   *Reference documentation* describes concepts and available functionality. A subset of this is API, or **Application Programming Interface**`\index{API (Application Programming Interface)}`{=latex}, documentation which is the detailed specification of the available functionality of a package, often consisting largely of docstrings like the previous example.
...
**Version control systems**`\index{version control}`{=latex} (VCS) refer to the tracking of changes to a codebase in a verifiable and coordinated way across project contributors. VCS underpins many aspects of automating the mundane parts of a modeler's job. Benefits of VCS include (either directly, or contribute significantly to):
...
Among several competing options (CVS, Mercurial, Subversion, etc.), Git`\index{Git}`{=latex} is the predominant choice as of this book's writing and therefore we will focus on Git concepts and workflows.
...
Each snapshot, or **commit**`\index{commit}`{=latex}, stores a reference to the complete state of the project — a tree covering all tracked files — along with metadata such as the author, a message, and a pointer to its parent commit(s)[^software-2]. Files that did not change between commits are not duplicated: identical content hashes to the same object and is stored only once. All of this is stored in a `.git/` subfolder of a project, which is automatically created upon initializing a repository. This folder may be hidden by default by your filesystem. You generally never need to modify anything inside the `.git/` folder yourself as Git handles this for you. Git tracks files in the repository working tree (the directory containing the .git folder and its subdirectories). It does not track directories outside the repository.

[^software-2]: Git uses a content-addressable filesystem, meaning it stores data as key-value pairs. The key is a hash of the content, ensuring data integrity and allowing efficient storage of identical content. For more on hashes, see the Content Hashes callout later in this chapter.
...
| end                               |                                           | `git add FileXYZ.jl`, then `git commit -m 'add pv function'`                                                               |                         |
| ```                               |                                           |                                                                                                                            |                         |
+-----------------------------------+-------------------------------------------+----------------------------------------------------------------------------------------------------------------------------+-------------------------+
...
::: callout-note
Some recommended Git tools with a **graphical user interface** (GUI):
...
Git is a distributed VCS, meaning that copies of the repository and all its history and content can live in multiple locations, such as on two colleagues' computers as well as a server. In this distributed model, what happens if you make a change locally?
...
Layered onto core Git functionality, services like Github provide interfaces which enhance the utility of VCS. A major example of this is **Pull Requests**`\index{pull request}`{=latex} (or PRs), which are a process of merging Git branches in a way that allows for additional automation and governance.
...
The features described here can take modelers many hours of time - testing, review of changes, sign-off tracking, etc. In this Git-based workflow, the process above happens frictionlessly and much of it is automated. This is such a powerful paradigm that it should be adopted within the financial industry and especially amongst modelers.
...
CI`\index{continuous integration (CI)}`{=latex} pipelines turn "remember to rerun the model" into code. A minimal Julia CI workflow usually runs:
...
Because GitHub Actions`\index{GitHub Actions}`{=latex}, GitLab CI, and Azure Pipelines capture logs and artifacts automatically, each pull request carries a timestamped record of which Julia version, OS, and dependency set produced the numbers. Regulators can accept these logs as part of model change evidence because they are immutable and repeatable.

### Data Version Control {#sec-data-version-control}

Git alone is not well suited for large, frequently changing binary files. Instead, the approach is to combine git with a **data version control**`\index{data version control}`{=latex} tool. These tools essentially replace the data (often called binary data or a blob) with a content hash in the git snapshots. The actual content referred to by the hash is then located elsewhere (e.g. a hosted server).

:::: {#nte-content-hash .callout-note}
## Content Hashes

**Content hashes**`\index{content hash}`{=latex} are the output of a function that transforms arbitrary data into a string of data. Hashes are very common in cryptography and in areas where security/certainty is important. Eliding the details of how they work exactly, what's important to understand for our purposes is that a content hash function will take arbitrary bits and output the same set of data each time the function is called on the original bits.
...
::: callout-tip
Use a cryptographic hash (e.g., SHA-256) to derive a stable, deterministic content hash. `Base.hash` is not a cryptographic hash and its values are not guaranteed to be stable across Julia versions, so it is not suitable for content-addressed storage.
:::
...
**Registries**`\index{registry}`{=latex} are a way to keep track of packages that are available to install, which is more complex than it might seem at first. The registry needs to keep track of:
...
Another case to consider is what if new functionality was *added*? Old code need not have changed, but if there's new functionality, how to communicate that as an API change? This is where **Semantic Versioning**`\index{semantic versioning}`{=latex} (SemVer) comes in: *semantic* means that your version of something is intended to convey some sort of meaning over and above simply incrementing from `v1` to `v2` to `v3`, etc.

#### Semantic Versioning

Semantic Versioning (SemVer) is one of the most popular approaches to software versioning. It's not perfect but has emerged as one of the most practical ways since it gets a lot about version numbering right. Here's how SemVer is defined[^software-3]:
...
::: callout-note
Numbers need not roll over to the next digit when they hit 10. That is, it's perfectly valid to go from `v1.9.0` to `v1.10.0` in SemVer.
:::

::: callout-tip
Sometimes you'll see a package with a version that starts with zero, such as `v0.23.1`. **We recommend that as soon as you register a package, you make it a `v1`**. `v1` need not indicate the package is "complete" (what software is?), so don't hold back on calling it `v1`. You're letting users install it easily, so you might as well call it the first version and move on!

According to SemVer's rules:

> Major version zero (0.y.z) is for initial development. Anything MAY change at any time. The public API SHOULD NOT be considered stable.

Most packages put an upper bound on compatibility so that major or minor changes in upstream packages are less likely to cause issues in their own packages. It can be somewhat painful to depend on a package which has a major version 0 (i.e. `v0.x.x`). You have to assume minor version bumps are backward incompatible changes, but the author might intend it to be a feature release. You should have skepticism of just upgrading to the new version of the dependency. It takes work on the downstream dependencies to decide if they should upgrade, adding mental and time loads to other authors and users.
...
::: callout-note
Utility Packages such as [ArtifactUtils.jl](https://github.com/JuliaPackaging/ArtifactUtils.jl) can assist in creating correct entries for `Artifacts.toml` files.
:::
...
A well-structured Julia package demonstrates key software engineering principles in action. The [JuliaTemplateRepo](https://github.com/bjack205/JuliaTemplateRepo.jl) repository demonstrates best practices for:
...
The [PkgTemplates.jl](https://github.com/JuliaCI/PkgTemplates.jl) package will allow you to create an empty repository with all of the testing, documentation, Git, and continuous integration scaffolding already in place.