...
The commit history forms a directed acyclic graph (DAG)`\index{DAG (directed acyclic graph)}`{=latex} representing the project's history. That is, there is an order to the history: the second commit is dependent on the first commit. From the second commit, two child commits may be created which both have the second commit as their parent. Each one of these commits represents a stored snapshot of the project at the time of the commit.
...
-   Versions of your package that your have made available and the Git hash associated with that version for provenance and tracking
