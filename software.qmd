...
Here's how SemVer is defined[^software-2]:
...
Also note that the numbers need not roll over to the next digit when they hit 10. `v2.32.7` is a perfectly valid SemVer version.

::: callout-tip
Sometimes you'll see a package with a version that starts with zero, such as `v0.23.1`. **We recommend that as soon as you register a package, to make it a `v1`**. `v1` need not inidcate the package is "complete" (what software is?), so don't hold back on calling it `v1`. You're letting users install it easily, so you might as well call it the first version and move on!
