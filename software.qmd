...
From this, it's possible to determine a score for how well a given set of code is tested. 100% coverage means every line of code has at least one test that double checked it's behavior.
...
::: callout-note
By convention, the primary branch of a project is named the `main` branch. Historically, this was called the `master` branch (after the [Master-Slave technology terms](https://en.wikipedia.org/wiki/Master–slave_(technology))). Due to connotations of human slavery most software will now default to the `main` name. However, many projects continue to use or have never renamed their original `master` branch.
:::
