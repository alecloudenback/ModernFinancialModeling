...
The above can be restructured to use GPU instead.
...
function gbm_simulation_gpu(μ, σ, S₀, Δt, num_steps, num_simulations)
    μ, σ, S₀, Δt = Float32.((μ, σ, S₀, Δt))
    stock_price_paths = MtlArray(zeros(Float32, num_steps, num_simulations))
    stock_price_paths .= S₀
    dW = MtlArray(randn(Float32, num_steps - 1, num_simulations) .* sqrt(Δt)) # <1>
    dS = μ * S₀ * Δt .+ σ .* S₀ .* dW
...
Axis(f[1, 1])
for i in 1:num_simulations
    lines!(1:num_steps, Array(gbm_paths[:, i]))
end
f
```

1. If using CUDA.jl, you would replace `using Metal` with `using CUDA` and `MtlArray(...)` with `CuArray(...)`.
