...
Pkg.instantiate()
using Statistics  # for quantile in scenario band plots
```
...
Scenario generators are widely used in risk management, investment analysis, and regulatory compliance to model potential future outcomes. For forecasting under the real-world (physical) measure `P`, use real-world (RW) scenarios`\index{real-world scenarios}`{=latex}. For valuation and hedging, use risk‑neutral (RN) scenarios`\index{risk-neutral scenarios}`{=latex} under the martingale measure `Q` (e.g., with drift adjusted to the short rate minus dividends for equities and to fit the initial curve for interest rates).
...
Short-rate models`\index{short-rate models}`{=latex} produce mean-reverting`\index{mean reversion}`{=latex} simulated rates that can be bootstrapped into full yield curves. They remain popular in ALM where projecting discount curves, reinvestment rates, and collateral flows is critical.

#### Vasicek and Cox-Ingersoll-Ross (CIR)

The Vasicek model`\index{Vasicek model}`{=latex} is a one-factor model commonly used for simulating interest rate scenarios. It describes the dynamics of short-term interest rates using a stochastic differential equation (SDE). In a Monte Carlo simulation, we can use the Vasicek model to generate multiple interest rate paths. The CIR model`\index{Cox-Ingersoll-Ross (CIR) model}`{=latex} is an extension of the Vasicek model with non-constant volatility. It is a square‑root diffusion with state‑dependent volatility that helps keep rates positive (for the continuous-time process, strictly so when the Feller condition $2\kappa\theta \geq \sigma^2$ holds and $r(0) > 0$).
...
The following code shows a simplified implementation of a CIR model, discretized with a truncated Euler–Maruyama`\index{Euler-Maruyama method}`{=latex} scheme. The specification of $dr$ can be changed to make it a Vasicek model.
...
let
    ...
    band!(ax, ti, p05, p95, color=(:steelblue, 0.2), label="5th–95th percentile")
    band!(ax, ti, p25, p75, color=(:steelblue, 0.4), label="25th–75th percentile")
...
::: callout-note
The positivity guarantee above belongs to the continuous-time process; the discretized simulation is another matter. Over a finite time step the Gaussian shock can push the rate negative even when the Feller condition holds, which is why the code clamps at zero (`max(r_prev, 0.0)` inside the square root, and truncation of the updated rate). This truncation is convenient and common in practice, but it means the code is an *approximation* of CIR: the clamping slightly biases the simulated distribution near zero, and the discretization error shrinks only as the time step is refined. The exact simulation scheme below avoids both issues.
:::

The CIR process can also be simulated exactly, because conditional on $r_t$ the distribution of $r_{t+\Delta t}$ is a scaled non-central chi-squared distribution.
...
let
    ...
    band!(ax, collect(tgrid), p05, p95, color=(:steelblue, 0.2), label="5th–95th percentile")
    band!(ax, collect(tgrid), p25, p75, color=(:steelblue, 0.4), label="25th–75th percentile")
    lines!(ax, collect(tgrid), p50, color=:steelblue, linewidth=2, label="Median")
    axislegend(ax, position=:rb)
    fig
end
```
...
-   $r(t)$ is the short-term interest rate at time $t$.
-   $θ(t)$ is the long-term mean or calibrated market-implied equilibrium level of the interest rate.
...
function hull_white_theta_timevar_sigma(years, prices, σt; a::Float64)
    ...
    # Compute integral term I(t_i) = ∫_0^{t_i} e^{-2a(t_i - s)} σ(s)^2 ds
...
let
    ...
    band!(ax, years, p05, p95, color=(:steelblue, 0.2), label="5th–95th percentile")
    band!(ax, years, p25, p75, color=(:steelblue, 0.4), label="25th–75th percentile")
...
let
    ...
    # Plot shaded bands
    f = Figure()
    ax = Axis(f[1, 1], xlabel="Trading days", ylabel="Price", title="GBM price scenarios")
    band!(ax, collect(ti), p05, p95, color=(:steelblue, 0.2), label="5th–95th percentile")
    band!(ax, collect(ti), p25, p75, color=(:steelblue, 0.4), label="25th–75th percentile")
    lines!(ax, collect(ti), p50, color=:steelblue, linewidth=2, label="Median")
...
GARCH models`\index{GARCH}`{=latex} capture time-varying volatility. They are often used in conjunction with other models to forecast volatility. The GARCH(1,1) model is defined as
...
function garch_simulation(rng::AbstractRNG, ω, α₁, β₁, num_steps, num_sims)
    ...
    for j in 1:num_sims
        volatility_paths[1, j] = sqrt(σ²_ss)
...
let
    ...
    Σ = [1.0 ρ;
        ρ 1.0]
    Cg = GaussianCopula(Σ)
    U = rand(Cg, 10_000)  # d × n matrix with U(0,1) margins
    f = Figure()
