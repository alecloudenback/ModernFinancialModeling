...
dt = T / Nsteps
nsim = 5 # number of simulated paths
...
θ(t) = 0.05 + 0.01 * sin(2π * t) # time-dependent mean level, illustrative only
σ(t) = 0.005 + 0.002 * cos(2π * t) # time-dependent volatility, illustrative only
...
for path in 1:nsim
    ...
    for i in 2:length(tgrid)
        t_prev = tgrid[i-1]
        dW = sqrt(dt) * randn()
        drift = (θ(t_prev) - a * r) * dt
        diffusion = σ(t_prev) * dW
        r += drift + diffusion
        rpaths[i, path] = r
    end
end
...
```{julia}
using Random, CairoMakie, Copulas
