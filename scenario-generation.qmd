...
Due to the nature of independence from different scenarios, all simulation examples can be restructured to use GPU instead.
...
# Function to simulate CIR process in gpu
function cir_simulation_gpu(κ, θ, σ, r₀, Δt, num_steps, num_simulations)
    interest_rate_paths = cu(zeros(num_steps, num_simulations))
    interest_rate_paths .= r₀
    dW = cu(randn(num_steps - 1, num_simulations)) .* sqrt(Δt)
    # for Vasicek
    # dr = κ * (θ - interest_rate_paths[i-1, j]) * Δt + σ * dW
    for i in 2:num_steps
        dr = κ .* (θ .- interest_rate_paths[i-1, :]) .* Δt .+ σ .* sqrt.(interest_rate_paths[i-1, :]) .* dW[i-1, :]
        interest_rate_paths[i, :] .= max.(interest_rate_paths[i-1, :] .+ dr, 0)  # Ensure non-negativity
    end
    return interest_rate_paths
end

# Run CIR simulation
cir_paths = cir_simulation_gpu(κ, θ, σ, r₀, Δt, num_steps, num_simulations)
...
# Function to simulate Hull-White process in gpu
function hull_white_simulation(α, σ, r₀, Δt, num_steps, num_simulations)
    interest_rate_paths = cu(zeros(num_steps, num_simulations))
    interest_rate_paths .= r₀
    dW = cu(randn(num_steps - 1, num_simulations)) .* sqrt(Δt)
    for i in 2:num_steps
        dr = α .* (σ .- interest_rate_paths[i-1, :]) .* Δt .+ σ .* dW[i-1, :]
...
# Function to simulate GBM in gpu
function gbm_simulation_gpu(μ, σ, S₀, Δt, num_steps, num_simulations)
    stock_price_paths = cu(zeros(num_steps, num_simulations))
    stock_price_paths .= S₀
    dW = cu(randn(num_steps - 1, num_simulations) .* sqrt(Δt))
    dS = μ * S₀ * Δt .+ σ .* S₀ .* dW
    stock_price_paths[2:end, :] .+= cumsum(dS, dims=1)
    return stock_price_paths
end

# Run GBM simulation
gbm_paths = gbm_simulation_gpu(μ, σ, S₀, Δt, num_steps, num_simulations)
...
# Function to simulate GARCH(1,1) volatility in gpu
function garch_simulation(α₀, α₁, β₁, num_steps, num_simulations)
    volatility_paths = cu(zeros(num_steps, num_simulations))
    ε = cu(randn(num_steps - 1, num_simulations))
    squared_returns = cu(zeros(num_simulations))
    for i in 2:num_steps
        squared_returns = α₀ .+ α₁ .* ε[i-1, :].^2 .+ β₁ .* squared_returns
        volatility_paths[i, :] .= sqrt.(squared_returns)
    end
    return volatility_paths
end
