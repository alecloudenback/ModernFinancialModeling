...
function cir_simulation(rng::AbstractRNG, κ, θ, σ, r₀, Δt, num_steps, num_simulations)
...
let
    ...
    f = Figure()
    ax = Axis(f[1, 1], xlabel="Trading days", ylabel="Short rate", tital="CIR paths")
    ti = 0:(num_steps-1)
...
function cir_exact_paths(rng::AbstractRNG, κ, θ, σ, r₀; horizon::Float64, steps::Int, sims::Int)
    dt = horizon / steps
    d = 4 * κ * θ / (σ * σ)
    c = (σ * σ * (1 - exp(-κ * dt))) / (4 * κ)
    rates = zeros(Float64, steps + 1, sims)
    time = range(0, horizon; length=steps + 1)
    rates[1, :] .= r₀
    for j in 1:sims
        r = r₀
...
let
    ...
    t, rpaths = cir_exact_paths(rng, κ, θ, σ, r₀; horizon=T, steps=nsteps, sims=nsims)
...
function garch_simulation(rng::AbstractRNG, α₀, α₁, β₁, num_steps, num_simulations)
    volatility_paths = zeros(num_steps, num_simulations)
    σ²_ss = ω / (1 - α₁ - β₁)
    returns = zeros(Float64, steps, sims)
    for j in 1:num_simulations
        returns[1, j] = volatility_paths[1, j] * randn(rng)
        for i in 2:num_steps
            σ² = α₀ + α₁ * returns[i - 1, j] * returns[i - 1, j] + β₁ * volatility_paths[i - 1, j] * volatility_paths[i - 1, j]
...
let
    ...
    garch_paths = garch_simulation(rng, α₀, α₁, β₁, num_steps, num_simulations)
