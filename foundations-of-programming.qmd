...
`let` blocks define variables within its scope and cannot be accessed outside thats scope. More on scope in @sec-scope.
```{julia}
#| error: true
c = let
    g = 3
...
[^foundations-of-programming-12]: Whether the last number is in the resulting range depends on if the step evenly divides the end of the range.
