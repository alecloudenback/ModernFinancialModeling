...
Note though, that `x` and `y` are both of the same type in each `Point` that we created. If instead we wanted to allow the coordinates to be of different types, then we could have defined `PPoint2D` as follows:
