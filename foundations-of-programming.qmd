...
We will dive a little bit deeper into variables and assignment in @sec-assignment.
...
The "Ship of Theseus[^foundations-of-programming-2]" problem is an example of how equality can be philosophically complex concept. In computer science we have the advantage that while we may not be able to resolve what's the "right" type of equality, we can be more precise about it.
...
[^foundations-of-programming-3]: This binary representations correspond to `B` and `66` with the *ASCII character set* and 8-bit integer encodings respectely, discussed later in this chapter.
...
-   Tuples are generally stack allocated instead of being heap allocated like arrays[^foundations-of-programming-12], meaning that a lot of times they can be faster than arrays.

[^foundations-of-programming-12]: What this means will be explained in @sec-hardware .
...
::: callout-tip
`Missing` and `Nothing` are the *types* while `missing` and `nothing` are the values here[^foundations-of-programming-13]. This is analogous to `Float64` being a type and `2.0` being a value.
:::

[^foundations-of-programming-13]: `Missing` and `Nothing` are instances of **singleton type,** which means that there is only a single value that either type can take on.
...
Notice the strings preceding the defintion of `dist`. In Julia, putting a string (`"..."`) or string literal (`"""..."""`) right above the definition will allow Julia to recognize the string as documentation and provided it to the user in help mode (@sec-help-mode) and/or have a documentation tool create a webpage or PDF documentation resource.
:::
