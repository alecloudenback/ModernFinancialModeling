...
Note though, that the `x` and `y` are both of the same type in each `Point` that we created. If instead we wanted to allow the coordinates to be of different types, then we could have defined `Point2D` as follows:
