...
**Computer Science**`\index{computer science}`{=latex} is the study of computing and information. As a science, it is distinct from programming languages which are merely coarse implementations of specific computer science concepts[^foundations-of-programming-1].
...
**Programming**`\index{programming}`{=latex} (or **"coding"**`\index{coding}`{=latex}) is the art and science of writing code in programming languages to have the computer perform desired tasks. While this may sound mechanistic, programming truly is one of the highest forms of abstract thinking. The design space of potential solutions is so large and potentially complex that much art and experience is needed to create a well-made program.
...
Within the code examples above, we can zoom in onto small pieces of code called **expressions**`\index{expression}`{=latex}. Expressions are effectively the basic block of code that gets evaluated to produce a value. Here is an expression that adds two integers together that evaluate to a new integer (`3` in this case):
...
**Conditionals**`\index{conditional}`{=latex} are expressions that evaluate to a **boolean**`\index{boolean}`{=latex} `true` or `false`. This is the beginning of really being able to assemble complex logic to perform useful work. Here are a handful of expressions that would evaluate to `true`:
...
The "Ship of Theseus problem"[^foundations-of-programming-2] is an example of how equality can be a philosophically complex concept. In computer science we have the advantage that while we may not be able to resolve what's the "right" type of equality, we can be more precise about it.
...
-   **Egal** equality`\index{equality!egal}`{=latex} is when a program could not distinguish between two objects at all

-   **Equal** equality`\index{equality!equal}`{=latex} is when the values of two objects are the same
...
When we have a mutable`\index{mutability}`{=latex} object (e.g. an `Array` or a `mutable struct`), we can mutate the value inside the referenced container. For example:
...
**Loops**`\index{loop}`{=latex} are ways for the program to repeat expressions while we want it to. There are two primary loops: `for` `\index{loop!for}`{=latex} and `while` `\index{loop!while}`{=latex}.
...
## Data Types {#sec-basic-data-types}

**Data types**`\index{data type}`{=latex} categorize information by intrinsic characteristics. We instinctively know that `13.24` is different than `"this set of words"`—types are how we formalize this distinction. Mathematically, it's like having different sets of objects to perform specialized operations on. Underneath this set-like abstraction are implementation details related to computer hardware. You probably know that computers only natively "speak" in binary zeros and ones. Data types are a primary way for a computer to know if it should interpret `01000010` as `B` or as `66`[^foundations-of-programming-3].

[^foundations-of-programming-3]: These binary representations correspond to `B` and `66` with the *ASCII character set* and 8-bit integer encodings respectively, discussed later in this chapter.

Each `0` or `1` within a computer is called a **bit**`\index{bit}`{=latex} and eight bits in a row form a **byte**`\index{byte}`{=latex} (such as `01000010`). This is where we get terms like "gigabytes" or "kilobits per second" as a measure of the quantity or rate of bits something can handle[^foundations-of-programming-4].
...
`0.2` will be interpreted as a 64-bit floating point type called `Float64` in Julia. Unlike integers, floating-point literals are always `Float64` regardless of system architecture. Having a finite number of bits attempting to represent a continuous, infinite set of numbers means that some numbers are not able to be represented with perfect precision. For example, if we ask for `0.2`, the closest representations in 64 and 32 bit are:

-   `0.20000000298023223876953125` in 32-bit

-   `0.200000000000000011102230246251565404236316680908203125` in 64-bit

This leads to special considerations that computers take when performing calculations on floating point numbers, some of which will be covered in more detail in @sec-hardware. For now, just note that floating point numbers have limited precision and even if we input `0.2`, your computations will use the above decimal representations even if it will print out a number with fewer digits shown:
...
::: callout-note
Note the difference in what printed between the last example and when we input `0.2` earlier in the chapter. The latter had the same (not-exactly equal to $0.2$) *value*, but it printed an abbreviated set of digits as a nicety for the user, who usually doesn't want to look at floating point numbers with their full machine precision. The system has the full precision (`0.20...3125`) but is truncating the output.
...
Additional range in the positive direction is available if one chooses to use "unsigned", non-negative numbers (`UInt32` and `UInt64`). Unlike floating point numbers, the integers have a type `Int` which will use the system bit architecture by default (that is, `Int(30)` will create a 64 bit integer on 64-bit systems and 32-bit on 32-bit systems).

::: callout-tip
## Floating Point and Excel

Excel's numeric behavior is close to, but not quite the same as, most programming languages, which follow the Institute of Electrical and Electronics Engineers' standards (such as the IEEE 754`\index{IEEE 754}`{=latex} standard for double precision floating point numbers). Excel stores and computes with IEEE 754 double precision (binary64) values internally, but it accepts only about 15 significant decimal digits of numeric entry (formatting can display more digit positions, though no additional precision), and it applies some documented deviations from pure IEEE 754 results in certain cases (for example, displaying exactly zero for some results that are merely very close to zero). In some ways this is the worst of both worlds: having the sometimes unusual (but well-defined) behavior of floating point arithmetic *and* having additional modifications layered on top. A programming language following IEEE 754 directly is not performing more "accurate" arithmetic — the underlying operations are essentially the same — but its results are well-defined and reproducible, without hidden adjustments between the computation and what you see.
...
Due to the inherent imprecision of floating point numbers, they should **not** be used in storing financial transaction records! The trade-offs inherent in floating point math described above do not lend themselves to accurate record-keeping. For example, consider summing up two products sold for \$19.99 and \$84.99: the floating point operation `19.99 + 84.99` does not precisely equal `104.98`.

```{julia}
19.99 + 84.99 == 104.98
```

Wrapping the result in a `BigFloat` reveals the exact value that was actually computed (note that this does not perform the addition in higher precision — the `Float64` addition happens first, and `BigFloat` simply displays that already-rounded result in full):

```{julia}
BigFloat(19.99 + 84.99)
```

See how the result is slightly lower than `104.98` — if we were adding US Dollars here, the stored total would disagree with the exact sum of the recorded amounts by a fraction of a cent. Any individual error is tiny, but a ledger that cannot exactly reproduce its own totals is a reconciliation problem waiting to happen once you scale to millions of transactions.
...
We can describe a *hierarchy* of types`\index{type hierarchy}`{=latex}. Both `Float64` and `Int64` are examples of `Real` numbers (here, `Real` is an **abstract** Julia type which corresponds to the mathematical set of real numbers commonly denoted with $\mathbb{R}$ ). So why not just define all numbers as a `Real` type? Because for performant calculations, the computer must know in advance how many bits each number is represented with.
...
Arrays`\index{array}`{=latex} are the most common way to represent a collection of similar data. For example, we can represent a set of integers as follows:
...
::: callout-note
In Julia, a `Vector{Float64}` `\index{vector}`{=latex} is simply a one-dimensional array of floating points and a `Matrix{Float64}` `\index{matrix}`{=latex} is a two-dimensional array. More precisely, they are **type aliases** of the more generic `Array{Float64,1}` and `Array{Float64,2}` names. Arrays with three or more dimensions don't have a type alias pre-defined.
:::
...
A **range**`\index{range}`{=latex} is a compact representation of a sequence of numbers. We actually used them above to index into arrays. They are expressed as `start:stop`.
...
```{julia}
sum(1:big(100_000_000_000_000_000_000_000_000_000_000_000_000_000))
```
...
**Characters**`\index{character}`{=latex} are represented in most programming languages as letters within quotation marks. In Julia, individual characters are represented using single quotes:

```{julia}
'a'
```

Letters and other characters present more difficulties than numbers to represent within a computer (think of how many languages and alphabets exist!), and it essentially only works because the world at large has agreed to a given representation. Originally **ASCII**`\index{ASCII}`{=latex} (American Standard Code for Information Interchange) was used to represent just 95 of the most common English characters ("a" through "z", zero through nine, etc.). Now, UTF (Unicode`\index{Unicode}`{=latex} Transformation Format) can encode more than a million characters and symbols from many human languages.

**Strings**`\index{string}`{=latex} are a collection[^foundations-of-programming-13] of characters, and can be created in Julia with double quotes:
...
Tuples`\index{tuple}`{=latex} are a fixed, ordered collection of values, denoted by values inside parentheses and separated by a comma. They are best suited for small collections of related data. An example might be x-y coordinates in 2 dimensional space:
...
-   Tuples are **immutable**, meaning that you cannot overwrite the value in memory (an error will be thrown if we try to do `p1[1] = 5`).
...
Named tuples`\index{named tuple}`{=latex} provide a way to give each field within the tuple a specific name. For example, our x-y coordinate example above could become:

```{julia}
p2 = (x=3, y=4)
```

The benefit is that we can give more meaning to each field and access the values in a nicer way. Previously, we used `p1[1]` to access the x-value, but with the new definition we can access it by name:
...
**Dictionaries**`\index{dictionary}`{=latex} are a container which relates a **key** to an associated **value**. Kind of like how arrays relate an index to a value, but the difference is that a dictionary is (1) un-ordered and (2) the key doesn't have to be an integer.
...
We just saw how tuples can contain heterogeneous types of data inside a common container. **Parametric Types**`\index{parametric type}`{=latex} are a way of allowing types themselves to be variable, with a wrapper type containing a to-be-specified inner-type.
...
`nothing` `\index{nothing}`{=latex} represents that there's nothing to be returned - for example if there's no solution to an optimization problem or if a function just doesn't have any value to `return` (such as in the case with input/output like `println`).

`missing` `\index{missing}`{=latex} is to represent a value that *should* be there but is not, as is all too common in real-world data. Julia natively supports `missing` and three-value logic, which is an extension of the two-value boolean (true/false) logic, to handle missing logical values:
...
[^foundations-of-programming-15]: `Missing` and `Nothing` are examples of a **singleton type,** which means that there is only a single value that either type can take on.

### Union Types

When two types may arise in a context, **union types**`\index{union type}`{=latex} are a way to represent that. For example, if we have a data feed and we know that it will produce *either* a `Float64` *or* a `Missing` type then we can say that the value for this is `Union{Float64,Missing}`. This is much better for the compiler (and our performance!) than saying that the type of this is `Any`.
...
`BasicPoint` is a **composite type**`\index{struct}`{=latex} because it is composed of elements of other types. Fields are accessed the same way as named tuples:
...
1. The `{T}` after the type's name allows for different `Point`s to be created depending on what the type of the underlying `x` and `y` is.
...
Both `p4` and `p5` are instances of different concrete types `Point{Int64}` and `Point{Float64}`. The expression `X isa Y` is `true` when `X` is a (sub)type of `Y`:
...
Julia has wonderful Unicode`\index{Unicode}`{=latex} support, meaning that it's not a problem to include characters like `θ`. The character can be typed in Julia editors by entering `\theta` and then pressing the TAB key on the keyboard.
...
**Constructors**`\index{constructor}`{=latex} are functions that return a data type (functions will be covered more generally later in the chapter). When we declare a `struct`, an implicit function is defined that takes a tuple of arguments and returns the data type that was declared. In the following example, after we define the `MyDate` `struct`, Julia creates a function (also called `MyDate`) which takes three arguments and will return the datatype `MyDate`:
...
And outer constructors are simply functions defined that have the same name as the data type, but are not defined inside the `struct` block. Extending the `MyDate` example, say we want to provide a default constructor for when no day is given such that the date returned is the 1st of the month:
...
Functions`\index{function}`{=latex} are a set of expressions that take inputs and return specified outputs.
...
**Unary operators**`\index{operator!unary}`{=latex} are operators which only take a single argument. Examples include the `!` which negates a boolean value or `-` which negates a number:

```{julia}
!true, -5
```

**Binary operators**`\index{operator!binary}`{=latex} take two arguments and are some of the most common functions we encounter, such as `+` or `-` or `>`:

```{julia}
1 + 2, 1 - 2, 1 > 2
```

The above unary and binary operators are special kinds of functions which don't require the use of parentheses. However, they can be written with parentheses for greater clarity:

```{julia}
!(true), -(5), +(1, 2), -(1, 2)
```

In Julia, a **function** defines behavior that maps a set of inputs to outputs. But a single function can adapt its behavior to the arguments themselves. We have just seen the function `-` be used in two different ways: negation and subtraction depending on whether it had one or two arguments given to it. In this way there is a conceptual hierarchy of functions that complements the hierarchy we have discussed in relation to types:

-   `-` is the overall function
-   `-(x)` is a unary function which negates its value, `-(x,y)` subtracts `y` from `x`
-   Specific methods are then created for each combination of concrete types: `-(x::Float64)` is a different method than `-(x::Int)`

**Methods**`\index{method}`{=latex} are specific compiled versions of the function for specific types. This is important because at a hardware level, operations for different types (e.g. integers versus floating point) differ considerably. By optimizing for the specific types Julia is able to achieve nearly ideal performance without the same sacrifices of other dynamic languages. We will develop more with respect to methods when we talk about dispatch in @sec-patterns-abstractions.
...
2.  We compute the distance formula for a point with x and y coordinates. The `return` value makes explicit what value the function will output.
...
This is referred to as **dispatching** on the argument types. Julia will look up to find the most specific method of a function for the given argument types, falling back to a generic implementation if one is defined.
...
Notice the strings preceding the definition of `dist`. In Julia, putting a string (`"..."`) or triple-quoted string (`"""..."""`) right above the definition will allow Julia to recognize the string as documentation and provide it to the user in help mode (@sec-help-mode) and/or have a documentation tool create a webpage or PDF documentation resource.
:::

::: callout-caution
## Defining Methods for Parametric Types

We learned that `Float64 <: Real` in the type hierarchy. However, note that `Vector{Float64}` is not a sub-type of `Vector{Real}`. This is called being **invariant** in type theory... but for our purposes this just practically means that when we define a method we need to specify that we want it to apply to all subtypes.

For example, `myfunction(x::Vector{Real})` would *not* be called if `x` was a `Vector{Float64}` because it's not a sub-type of `Vector{Real}`. To act the way we want, we would define the method with the signature of `myfunction(x::Vector{<:Real})` or `myfunction(x::Vector{T}) where {T<:Real}`.

(Tuples are the one exception to this rule: `Tuple{Float64}` *is* a sub-type of `Tuple{Real}`.)
:::

### Keyword Arguments

**Keyword arguments**`\index{keyword argument}`{=latex} are arguments that are passed to a function using named identifiers rather than position. In the following example, `filepath` is a **positional argument** while the two arguments after the semicolon (`;`) are keyword arguments.
...
**Anonymous functions**`\index{anonymous function}`{=latex} are functions that have no name and are used in contexts where the name does not matter. The syntax is `x -> ...expression with x...`. As an example, say that we want to create a vector from another where each element is squared. `map` applies a function to each member of a given collection:
...
Functions in many languages including Julia are **first class**`\index{first-class function}`{=latex} which means that functions can be assigned and moved around like data variables.
...
Looking at the prior definition of `dist`, what if we wanted to compute the squared distance from the origin for a set of points? If those points are stored in an array, we can **broadcast**`\index{broadcasting}`{=latex} functions to all members of a collection at the same time. This is accomplished using the **dot-syntax** as follows:
...
Effectively, broadcasting paired up each element of the vector with the individual entries of the corresponding row of the matrix, producing a matrix of element-wise comparisons:

``` julia
in(1, 1) in(1, 2) in(1, 3) # first element checked against first row's entries
in(9, 4) in(9, 5) in(9, 6) # second element checked against second row's entries
```
...
We often want to share data between scopes, such as between modules or by passing data into a function's scope. Arguments to a function in Julia are **passed-by-sharing**`\index{pass-by-sharing}`{=latex} which means that an outside variable can be mutated from within a function. We can modify the array in the outer scope (scope discussed later in this chapter) from within the function. In this example, we modify the array that is assigned to `v` by doubling each element:
...
In projects of even modest complexity, it can be challenging to come up with unique identifiers for different functions or variables. **Scope**`\index{scope}`{=latex} refers to the bounds for which an identifier is available. We will often talk about the **local scope**`\index{scope!local}`{=latex} that's inside some expression that creates a narrowly-defined scope (such as a `function` or `let` block) or the **global scope**`\index{scope!global}`{=latex} which is the top level scope that contains everything else inside of it[^foundations-of-programming-17].

[^foundations-of-programming-17]: A `module` block (discussed below) also encapsulates its contents, but technically it introduces a new *global* scope rather than a local one. Here are a few examples to demonstrate scope.
...
In fact, if we try to use `j` outside of the scope defined above we will get an error[^foundations-of-programming-16].

[^foundations-of-programming-16]: Note that this error **stack trace** is unfortunately much messier due to it running in a Quarto notebook environment. When running scripts or in a REPL, your stack traces should look much simpler and easier to follow the error's lineage.
...
**Modules**`\index{module}`{=latex} are ways to encapsulate related functionality together. Another benefit is that the variables inside the module don't "pollute" the **namespace**`\index{namespace}`{=latex} of your current scope. Here's an example:
...
-   A **package**`\index{package}`{=latex} is a module that has a specific set of files and associated metadata. Essentially, it's a module with a `Project.toml` file that has a name and unique identifier listed, and a file in a `src/` directory called `MySimulation.jl`
