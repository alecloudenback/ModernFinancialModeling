...
We have, of course, not defined an exhaustive list of operations, covering only `+`, `*`, `exp`, `sin`, and `cos`. There are only a few more arithmetic operations (`-`, `/`) and transcendental functions (`log`, more trigonometric functions, etc.) to define before we would have a very robust set of algebraic operations for our `DualNumber`. In fact, it's possible to go even further and to define the behavior through conditional expressions and iterations to differentiate fairly complex functions or to extend the mechanism to partial derivatives and higher-order derivatives as well.
...
With the above code, now we can get the partial derivatives with respect to each parameter. The first, third, fourth, fifth, and sixth correspond to the common "Greeks"`\index{Greeks (finance)}`{=latex} *delta*, *theta* (up to a sign convention, noted below), *rho*, *vega*, and *phi* respectively. The second term is the partial derivative with respect to the strike price:
...
let
    g = similar(params) #<1>
    cfg = ForwardDiff.GradientConfig(eurocall, params) #<2>
    @btime ForwardDiff.gradient!($g, eurocall, $params, $cfg)
end
```

1.  Pre-allocate the output array so the benchmark measures only the gradient computation.
2.  Pre-allocate the `GradientConfig` to avoid ForwardDiff re-creating internal buffers on each call — without this, the overhead is dominated by allocations rather than actual derivative computation.
...
Forward-mode AD computes all six first-order derivatives faster than finite differences and returns exact results — no step-size tuning and no truncation error (recall @fig-finite-diff-error).

## Forward Mode and Reverse Mode

The approach outlined above is forward-mode AD`\index{forward mode (automatic differentiation)}`{=latex}, where derivatives are propagated alongside values. Reverse-mode AD`\index{reverse mode (automatic differentiation)}`{=latex}`\index{backpropagation}`{=latex} evaluates the function, records a computation graph, and then accumulates sensitivities backwards.
...
-   [**DifferentiationInterface.jl**](https://github.com/JuliaDiff/DifferentiationInterface.jl)`\index{DifferentiationInterface.jl}`{=latex} is a wrapper library providing a consistent API while being able to exchange different backends.
