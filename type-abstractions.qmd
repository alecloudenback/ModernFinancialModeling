...
2.  Define how the assets are to be valued.
...
5.  **Enable type safety**. By specifying the expected types for function arguments, we can catch type-related errors immediately at the call site (as a `MethodError`) rather than letting bad data propagate deeper into a calculation. This helps prevent bugs and makes our code more robust.

With these benefits in mind, let's start by defining the types for our assets. We'll create an abstract type called `AbstractAsset` that will serve as the parent type for all our asset types. If you haven't read it already, @sec-user-defined-types is a good reference for details on types at the language level (this section is focused on organization and building up the abstracted valuation process).
...
Here's the outline of what follows to get an understanding of types, type hierarchy`\index{type hierarchy}`{=latex}, and multiple dispatch.
...
Now on to defining the valuation for `Cash` and `AbstractBond`s. Cash is always equal to its balance:
...
Despite the definitions above, the following will error because we haven't defined a method for `value` which takes a bond together with an `EconomicAssumptions` as its second argument:
...
1.  The `r::Function` constraint says to use this method if `r` is an instance of any subtype of the (abstract) `Function` type.
...
This is an issue with traditional OO designs, and one that resolves itself so elegantly with multiple dispatch.
...
We run into limitations with the Python version. We can extend a new representation (dataclass), but adding new functionality (e.g. `cash_value`) requires modifying other classes which you may not own and for which the method may not apply.
