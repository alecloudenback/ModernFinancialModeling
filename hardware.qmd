...
-   The **stack**`\index{stack}`{=latex} stores small, fixed-size (known bit length) data and program components. The stack is a last-in-first-out queue of data that is able to be written to and read from very quickly. The stack is CPU cache friendly.
...
Logical circuits (**transistors**`\index{transistors}`{=latex}) are an arrangement of wires that output a new electrical signal that varies depending on the input. From a collection of smaller building block gates (e.g. AND, OR, NOR, XOR) more complex operations can be built up[^hardware-1], into operations like addition, multiplication, division, etc. Electric impulses move the state of the program forward once per CPU cycle (controlled by a master "clock" ticking billions of times per second). CPU cycle speed is what's quoted for chip performance, e.g. when a CPU is advertised as 3.0 GHz (or 3 billion cycles per second).
