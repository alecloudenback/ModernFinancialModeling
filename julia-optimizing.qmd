...
This phenomenon is called "type instability".
...
To find bottlenecks in a larger program, you should rather use a profiler(see next section) or the package [TimerOutputs.jl](https://github.com/KristofferC/TimerOutputs.jl).
...
Type-stability is a contagous thing: if a variable's type cannot be inferred, then the types of variables that depend on it may not be inferrable either.
...
This is akin to repeatedly calling `@code_warntype` deeper and deeper into your functions ("slowly succumbing to the madness..." of type instability).
...
:::callout-tip
To be more forceful about ensuring type stabiltiy in your code, one approach is to error whenever a type instability occurs: the macro `@stable` from [DispatchDoctor.jl](https://github.com/MilesCranmer/DispatchDoctor.jl) allows exactly that.
:::
...
In particular, try to modify existing arrays instead of allocating new objects and try to access arrays in the right order for Julia, i.e. accessing data down columns instead of across rows.
...
It allows module authors to specify methods to precompile when a module is loaded for the first time. The methods chosen should represent those which would be used during typical user workflows.
These methods then have the same latency as if they had already been run by the end user. This adds upfront pre-compilation time when installing a package version for the first time, but then subsequent uses will be much quicker.
...
To reduce the time that packages take to load, you can use [PackageCompiler.jl](https://github.com/JuliaLang/PackageCompiler.jl) to generate a custom version of Julia, called a **sysimage** (system image), with its own custom standard library.
...
The helpful [translation guide](https://juliafolds2.github.io/OhMyThreads.jl/stable/translation/) demonstrates how Base multi-threading loops can be translated into the OhMyThreads API.
