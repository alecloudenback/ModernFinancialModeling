...
d₁₂ = LinearAlgebra.norm(Array(df_num[:, 1]) .- Array(df_num[:, 2]), 1)
```
...
k-Nearest Neighbor (kNN)`\index{k-nearest neighbor (kNN)}`{=latex} is primarily known as a classification algorithm, but nearest-neighbor queries are a building block for many similarity-driven tasks: classification, recommendation, anomaly detection, and density-based clustering methods. Given a query point, we find the $k$ closest points under a chosen metric; a KD-tree`\index{KDTree}`{=latex} makes these lookups efficient by recursively partitioning the space so that most candidates can be ruled out without computing their distances.
