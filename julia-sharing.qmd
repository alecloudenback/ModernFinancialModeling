...
[Runic.jl](https://github.com/fredrikekre/Runic.jl) is a popular choice as well. Like Python's popular Black formatter, there is no configuration to the formatting options when using Runic. The benefit is increased consistency and no time wasted debating formatting decisions. 

Runic requires you to set up Runic using Pkg. The instructions are straightforward and available on the [Runic.jl repository](https://github.com/fredrikekre/Runic.jl).
...
In addition to docstrings, [Documenter.jl](https://github.com/JuliaDocs/Documenter.jl)\index{Documenter.jl}`{=latex}` allows you to design a website for all of this, based on Markdown files contained in the `docs` subfolder of your package. Unsurprisingly, its own [documentation](https://documenter.juliadocs.org/stable/) is excellent and will teach you a lot. To build the documentation locally, just run
