...
**`map`** is so named for the mathetmatical concept of mapping an input to an output. Here, it's effectively the same thing. We take a collection and use the given function to calculate an output. The size of the output equals the size of the input.
...
`map` transforms the `rate` collection by applying the anonomyous function `x -> 1 / (1 + x)`, which is the single period discount factor. This operation is conveyed visually in @fig-map-diagram.
...
`accumulate` takes an operation and a collection and returns a collection where each element is the cumulative result of applying the operation from the first element to the current one.
For example, to calculate the cumulative product of the one-period discount factors:
```{julia}
...
Another example beyond addition and subtraction is string concatenation. In Julia, two strings are concatenated with `*` (like in mathematics, $a * b$ is also written as $ab$). The identitiy element for strings where the binary operation $\bigodot = *$ is `""`. For example:
...
`filter` does what you might think - filter a collection based on some critereon that can be determined as true or false.
...
The rest of this chapter will be focused on how we will assemble a set of  interfaces that let's us value a portfolio of assets. Using the constructs introduced in this chapter, we can describe this as additively reducing over the value-mapped collection of assets in the portfolio. Or more concisely:
...
We will define four types of assets in this simplified universe:

1. Cash
2. Risk Free Bonds
3. European Puts and Calls
...
::: callout-tip
In the preceding example, we used types that contained all of the information about how to value them. However, in a more complete implementation it would be better to separate out the economic conditions from the details of the asset itself. For example, if we wanted to define value as a two argument function which took economic conditions, then we could have defined the pieces like this:

```julia
struct EconomicConditions
   volatility::Float64
   risk_free::Float64
end
function value(c::EuroCall,econ::EconomicConditions)
