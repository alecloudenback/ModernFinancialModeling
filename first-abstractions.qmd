...
```{julia}
discount_factors(rates) = accumulate(
...
To complete the example with functional building blocks, we 

```{julia}
map(*, cf_bond, discount_factors(rate))
```

```{julia}
mapreduce(*, +, cf_bond, discount_factors(rate))
```
