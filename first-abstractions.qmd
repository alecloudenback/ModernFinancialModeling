...
```{julia}
function pv_recursive(rate,cashflows,accumulated_value=0.0,discount_factor=1.0)
  if isempty(cashflows) #<1>
    return accumulated_value
  else
    discount_factor = discount_factor / (1+rate) # <2>
    av = (accumulated_value + first(cashflows)) * discount_factor # <3>
    return pv_recursive(rate,cashflows[begin+1:end], av,discount_factor) # <4>
  end
end
