...
```
: A diagram showing that `map` creates a new collection mirroring the old one, after applying the given function to each element in the original collection. {#tbl-map-diagram}
...
In the following example, we take two collections and multiply them together. Specifically, we'll pair up each cashflow with the associated discount factor (this works because the arrays are in the correct time-based order for our problem).
