...
```{julia}
using TSne, DataFrames, Random, Distributions, Plots, StatsBase, ScikitLearn
...
df = DataFrame(
    ...
    Market_Cap = rand(Uniform(1, 500), num_stocks),  # in billion USD
...
# Apply t-SNE
tsne_result = tsne(X; d=2, perplexity=30, max_iter=1000)
...
f = Figure()
axis = Axis(f[1, 1])
scatter!(axis, df.TSNE_1, df.TSNE_2, color=Int.(allabels[1:size(tsne_result, 1)]))
f
```
