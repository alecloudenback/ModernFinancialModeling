...
The circles compound the problem. People intuit comparisons of area, not diameter — so the breast cancer funding circle *appears* roughly three times as large as the prostate cancer one, even though it represents less than twice the money. Disease names should be placed directly on the circles rather than in a separate legend, both for readability and for accessibility (color-blind readers can't rely on hue alone). The numerical labels also carry more precision than the story needs — deaths to the exact person and dollars to five significant figures — which is noise, not signal.
