...
Visualization transforms raw data into something we can see and understand—making it easier to uncover patterns, communicate ideas, and make decisions.
...
How does one develop effective visualizations? While a specialty all its own, we present the following considerations when creating quantitative displays of visual information. Consider this a guide for how to create visualizations of data.
...
-   **Think about human perception of shapes. For example:**
...
-   **Comparison Clarity**: To effectively compare funds raised to deaths caused, these metrics should be displayed side-by-side or connected with lines. The size of the circles is also misleading: using circle diameters to represent values distorts perception because people intuit a comparison of area, not diameter. For example, the breast cancer money raised circle is nearly 4 times as large as the prostate cancer one, even though it raised less than twice as much money.
...
The new visualization is easier to understand and draw comparisons. From the clarity, relationships are revealed and the visualization itself reveals interesting followup questions and suggests follow-on analysis to be performed.
...
-   Basic Charts and Graphs: Bar charts, line graphs, scatter plots, histograms, pie charts.
    -   Bar charts`\index{bar chart}`{=latex} are best for comparing categorical data or discrete values across different categories. Sometimes categories can be grouped for a stacked bar chart to show, for example, how each category changes over time.
    -   Line graphs`\index{line chart}`{=latex} are best for showing trends over time or when we want to highlight the rate of change. It is very intuitive to use line graphs to track trends or patterns.
    -   Scatter plots`\index{scatter plot}`{=latex} are best for showing relationships or correlations between two variables. It is used a lot when one looks for patterns, clusters or outliers, or would like to explore the distribution of data points across different dimensions.
    -   Histograms`\index{histogram}`{=latex} are best for showing the distribution of a single continuous variable, or visualizing the distribution of data points across different ranges or intervals.
...
-   Multivariate Visualizations: Heatmaps`\index{heatmaps}`{=latex}, parallel coordinates plots, radar charts, bubble charts.
...
-   Dimensionality Reduction: PCA plots, t-SNE`\index{t-SNE}`{=latex}, and UMAP for visualizing high-dimensional data. Here we show an example of how high-dimensional data can be shown on a t-SNE plot, clustering synthetic stocks based on financial indicators like:
...
-   Geospatial Visualization: Maps, choropleth maps`\index{choropleth maps}`{=latex} for visualizing spatial data. Here we show how spatial data can be visualized using a choropleth map.
...
Julia has several powerful visualization packages, each with different strengths (interactive vs. static, ease of use vs. customization). The best choice depends on your needs, data type, and whether you want interactive or static output. Here are the most common ones:

### CairoMakie.jl and GLMakie.jl

`Makie` `\index{Makie.jl}`{=latex} is designed for high-performance, interactive, and 3D visualization. It supports real-time interaction and is highly customizable. It supports 2D and 3D plotting and real-time interactivity. It is also extremely fast with GPU acceleration for certain operations. CairoMakie`\index{CairoMakie.jl}`{=latex} is suitable for print-quality vector output, while GLMakie utilizes GPU acceleration for high quality 2D and 3D plots. CairoMakie was chosen as the tool for this book because it offers very sensible default behavior and aesthetics, is easy to customize, and generally straightforward code.

### Plots.jl

`Plots` `\index{Plots.jl}`{=latex} is one of the most versatile and popular Julia plotting libraries. It provides a high-level interface for different plotting backends (e.g., GR, Plotly, PyPlot, PGFPlotsX, etc.). It uses a high-level syntax that is easy to use. It supports multiple backends for both static and interactive plots, but can be more limited in its customization options.

#### StatsPlots.jl

`StatsPlots` extends `Plots` by adding statistical plot types such as boxplots`\index{boxplot}`{=latex}, violin plots`\index{violin plot}`{=latex}, histograms, and density plots. It's ideal for users who frequently work with statistical data. It is specialized for statistical visualizations. It allows easy integration with Julia’s statistical packages like `DataFrames` and `StatsBase`.

### GraphPlot.jl

This package is used to plot graphs (networks), such as social network visualizations or other graph-related problems. It supports integration with the `Graphs.jl` package for graph analytics.
